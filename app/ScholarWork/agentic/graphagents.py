...
from app.ScholarWork.shared.models import ModelOutput
...
class GraphScholarWorkAgents:
    """LangGraph-based multi-agent implementation for ScholarWork."""
    ...
    def specialist_node(self, state: AgentState):
        prompt = f"You are a Specialist for ScholarWork. Analyze this input: {state['input']}"
        response = self.llm.invoke(prompt)
        return {"specialist_output": response.content}

    def verifier_node(self, state: AgentState):
        prompt = f"You are a Verifier for ScholarWork. Interrogate this analysis: {state['specialist_output']}"
        response = self.llm.invoke(prompt)
        return {"verifier_output": response.content}

    def manager_node(self, state: AgentState):
        prompt = f"""Synthesize the following into a final Markdown report for ScholarWork:
        Input: {state['input']}
