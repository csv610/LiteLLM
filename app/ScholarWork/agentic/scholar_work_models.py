...
class ResearchBrief(BaseModel):
    ...
    major_contributions: List[str] = Field(..., description="A comprehensive list of the specific major works or breakthroughs")
    historical_context: str = Field(..., description="The scientific and social landscape during the time of discovery")
    scientific_core: str = Field(..., description="The fundamental scientific principles and logic behind the major discoveries")
    revolutionary_impact: str = Field(..., description="How these works changed existing paradigms or understanding")
    modern_legacy: List[str] = Field(..., description="3-5 concrete ways this work influences science or society today")
    key_anecdotes: List[str] = Field(..., description="Potential human-interest stories or anecdotes related to the discoveries")

    @model_validator(mode="before")
    @classmethod
    def _coerce_legacy_fields(cls, data):
        if isinstance(data, dict) and "major_contributions" not in data and data.get("major_contribution"):
            data = dict(data)
...
class SynthesizedReport(BaseModel):
    ...
    scholar_name: str = Field(..., description="The name of the scholar")
    contributions: List[str] = Field(..., description="A list of detailed technical descriptions of major works")
...
class ScholarMajorWork(BaseModel):
    ...
    contribution_list: str = Field(..., description="A comprehensive, structured list of major works and contributions with detailed explanations for each.")

    # Supporting materials
    key_terms: str = Field(..., description="Brief explanations of technical terms or concepts used in the report")
    impact_summary: str = Field(..., description="A summary of the long-term impact of these works on science or society")
    ...
    @property
    def major_contribution(self) -> str:
        if not self.contribution_list: return self.title
