...
from app.Paradox.shared.models import ModelOutput
...
class GraphParadoxAgents:
    """LangGraph-based multi-agent implementation for Paradox."""
    ...
    def specialist_node(self, state: AgentState):
        prompt = f"You are a Specialist for Paradox. Analyze this input: {state['input']}"
        response = self.llm.invoke(prompt)
        return {"specialist_output": response.content}

    def verifier_node(self, state: AgentState):
        prompt = f"You are a Verifier for Paradox. Interrogate this analysis: {state['specialist_output']}"
        response = self.llm.invoke(prompt)
        return {"verifier_output": response.content}

    def manager_node(self, state: AgentState):
        prompt = f"""Synthesize the following into a final Markdown report for Paradox:
        Input: {state['input']}
