"""
liteagents.py - Unified LiteClient-based agents for Paradox.
"""
...
class ParadoxExplainer:
    ...
    def _researcher_agent(
        self, paradox_name: str, level: AudienceLevel
    ...
    def generate_text(
        self, paradox_name: str, audience_levels: List[AudienceLevel] = None
        ...
        for level in audience_levels:
            self.logger.info(
                f"Tier 1/2: Processing '{paradox_name}' for {level.value} level..."
            )
            ...
            if research and logic and domain:
                final_explanation = self._orchestrator_agent(
                    paradox_name, level, research, logic, domain
                )
        ...
        synth_prompt = (
            ...
            + "\n\n".join(
                [
                    f"### For {l.value}:\n{e.introduction}\n\n**Contradiction:** {e.the_contradiction}"
                    for l, e in explanations.items()
                ]
            )
        )
