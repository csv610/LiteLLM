...
from PeriodicTable.nonagentic.periodic_table_models import (
    ElementInfo,
...
def test_periodic_table_element_init(mock_model_config):
    ...
    with patch(
        "PeriodicTable.nonagentic.periodic_table_element.LiteClient"
    ) as MockClient:
...
def test_fetch_element_info_success(mock_model_config, element_info_sample):
    ...
    with patch(
        ...
        mock_client_instance.generate_text.return_value = ElementResponse(
            element=element_info_sample
        )
...
def test_get_element_summary(mock_model_config, element_info_sample):
    ...
    with patch("PeriodicTable.nonagentic.periodic_table_element.LiteClient"):
        pte = PeriodicTableElement(mock_model_config)
