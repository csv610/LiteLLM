...
def run_live_test(procedure_name: str, model_name: str = "ollama/gemma3"):
    ...
    for i, t in enumerate(triples):
        print(
            f"  [{i + 1}] {t.source} ({t.source_type}) --({t.relation})--> {t.target} ({t.target_type})"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Live test for Procedure Knowledge Graph"
    )
    parser.add_argument(
        "--procedure", type=str, default="Knee Replacement", help="Procedure to test"
    )
    parser.add_argument(
        "--model", type=str, default="ollama/gemma3", help="LLM model to use"
    )
