...
def test_live_build_from_medicine():
    """Live test that calls the LLM to generate medicine triples."""
    ...
    triples = builder.build_from_medicine(medicine_name)
    
    # Basic assertions to ensure we got a valid response
    assert len(triples) > 0, "No triples generated from live model."
    
    # The source medicine should be present in the generated nodes
    nodes_lower = [str(n).lower() for n in builder.G.nodes]
    assert any(medicine_name.lower() in n for n in nodes_lower), f"Expected '{medicine_name}' to be present in graph nodes."
    
    # Verify that relations and entities are valid (they should have been validated by the Pydantic model)
