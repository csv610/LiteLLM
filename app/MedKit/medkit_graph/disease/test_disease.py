import pytest
import json
from disease_models import Triple, DiseaseGraphBuilder, DiseaseTripletExtractor
...
def test_triple_normalization():
    ...
    t4 = Triple(source="Malaria", relation="has_symptom", target="Fever", source_type="disease", target_type="sign")
...
def test_graph_builder():
    ...
    triples = [
        ...
        Triple(source="Malaria", relation="has_symptom", target="Chills", source_type="Disease", target_type="Symptom"),
        Triple(source="Malaria", relation="treated_with", target="ACT", source_type="Disease", target_type="Treatment"),
        Triple(source="Malaria", relation="affects_organ", target="Liver", source_type="Disease", target_type="Organ"),
    ]
...
def test_extractor_simulation():
    ...
    has_fever = any(t.target == "Fever" and t.relation == "has_symptom" for t in triples)
    assert has_fever
