"""
graphagents.py - Multi-agent implementation using LangGraph for tests.
"""
...
from app.MedKit.tests.shared.models import ModelOutput
...
class GraphTestsAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for tests: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for tests synthesize: {state['specialist_output']}")
