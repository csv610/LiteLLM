...
class ChatMessage(BaseModel):
    ...
    red_flags_detected: List[str] = Field(
        default=[], description="Any red flags detected in message"
    )
...
class ChatSession(BaseModel):
    ...
    messages: List[ChatMessage] = Field(
        default=[], description="All messages in session"
    )

    # Assessment data
    assessment_data: Optional["MentalHealthAssessment"] = Field(
        None, description="Final assessment from session"
    )

    # Status
    session_status: str = Field(
        "active", description="active, completed, emergency, or terminated"
    )
    emergency_triggered: bool = Field(
        False, description="Emergency protocols activated"
    )
...
class PrivacyConsent(BaseModel):
    ...
    data_retention_understood: bool = Field(
        description="Understood data retention practices"
    )
    hipaa_notice_acknowledged: bool = Field(
        description="Acknowledged HIPAA notice of privacy practices"
    )
    data_sharing_consent: bool = Field(
        description="Consent to share data with healthcare providers if needed"
    )
    research_consent: bool = Field(
        False, description="Consent to anonymized research use"
    )
...
class AuditLog(BaseModel):
    ...
    action: str = Field(
        description="Action performed: data_access, data_modification, data_deletion, assessment_created, report_generated"
    )
