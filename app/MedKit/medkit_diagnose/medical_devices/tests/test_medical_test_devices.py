...
class TestMedicalTestDeviceGuide(unittest.TestCase):
    ...
    def test_generate_text_unstructured(self, mock_prompt_builder, mock_model_config, mock_lite_client):
        # Setup
        model_config = MagicMock()
        model_config.model = "test-model"
        guide = MedicalTestDeviceGuide(model_config)
    ...
    @patch('medical_test_devices.ModelConfig')
    @patch('medical_test_devices.PromptBuilder')
    def test_generate_text_structured(self, mock_prompt_builder, mock_model_config, mock_lite_client):
