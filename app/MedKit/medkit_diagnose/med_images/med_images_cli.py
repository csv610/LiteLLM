...
def get_user_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "input", type=str, help="Image file path or directory of images."
    )
    ...
    parser.add_argument(
        ...
        default=3,
        choices=[0, 1, 2, 3, 4],
        help="Logging verbosity (default: 3).",
    )
    parser.add_argument(
        "-s",
        "--structured",
        action="store_true",
        default=True,
        help="Use structured output (default: True).",
    )
...
def create_med_images_report(args) -> int:
    ...
    try:
        ...
        pbar = tqdm(
            ...
            desc="Classifying",
            unit="image",
            disable=len(items_to_process) <= 1,
        )
        for item in pbar:
