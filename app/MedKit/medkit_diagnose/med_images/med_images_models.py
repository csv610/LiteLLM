"""
med_images_models.py - Pydantic models for med image classification information

Defines the data structures used for comprehensive, evidence-based med image classification
documentation.
"""
...
class MedImagesModel(BaseModel):
    """Comprehensive model for med image classification information."""
    # Basic Information
...
class MedicalImageClassificationModel(BaseModel):
    """Model for medical image classification results."""
    modality: str = Field(description="The medical imaging modality (X-Ray, CT-Scan, Ultrasound, MRI, mammography, histopathology, retinal fundus, dermoscopic images, SPECT).")
    anatomical_site: str = Field(description="The body part or organ being imaged.")
    findings: str = Field(description="Detailed clinical findings observed in the image.")
    classification: str = Field(description="The primary classification or diagnosis based on the image.")
    confidence_score: float = Field(description="Confidence score for the classification (0.0 to 1.0).")
    recommendations: str = Field(description="Recommended next steps or follow-up tests.")

class ModelOutput(BaseModel):
    data: Optional[Union[MedImagesModel, MedicalImageClassificationModel]] = None
