...
from sane_interview import Question, SANEInterview
...
def test_consent_full_flow():
    ...
    q_medical_info_intro = interview.get_next_question(
        "yes"
    )  # Returns Info: To provide best medical care...
...
def test_sexual_contact_parsing():
    ...
    assert (
        SexualContactType.ANAL not in interview.interview.sexual_contact.contact_types
    )
...
def test_medical_history_conditional_age():
    ...
    q_last = flow.send(
        ""
    )  # allergies -> if age < 12, this should raise StopIteration after this send
