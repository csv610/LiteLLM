...
from llm_sane_interview import InterviewContext, LLMAssistedSANEInterviewer
...
def test_get_response_with_llm_assist_accepted():
    mock_io = MockIO(
        responses=["I was choked.", "1"]
    )  # 1. Response, 2. Accept first suggestion
    interviewer = LLMAssistedSANEInterviewer(use_llm_assist=True, io_handler=mock_io)

    # Mock the follow-up response
    with patch.object(
        mock_io, "ask", side_effect=["I was choked.", "1", "No, I can breathe fine."]
    ):
        response = interviewer.get_response_with_llm_assist(
            "What happened to you?", section="injury_assessment"
        )
...
def test_get_response_with_llm_assist_skipped():
    mock_io = MockIO(
        responses=["I was choked.", "s"]
    )  # 1. Response, 2. Skip suggestions
    interviewer = LLMAssistedSANEInterviewer(use_llm_assist=True, io_handler=mock_io)
