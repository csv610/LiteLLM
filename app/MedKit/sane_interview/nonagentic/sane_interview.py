...
from sane_interview_models import SANEInterviewRecord, SexualContactType, YesNoUnsure
...
class SANEInterview:
    ...
    def intro_questions(self):
        ...
        patient_id = yield Question(
            "Enter patient identifier (anonymous):", allow_skip=False
        )
    ...
    def consent_questions(self):
        ...
        yield Question(
            "\nTo provide the best medical care, I need to collect some basic information.",
            type="info",
        )
        ...
        if self._parse_yes_no(ans) == YesNoUnsure.YES:
            ans = yield Question(
                "What sex were you assigned at birth? (male, female, or intersex)",
                allow_skip=False,
            )
            if ans:
    ...
    def incident_questions(self):
        ...
        ans = yield Question(
            "Were there any witnesses or anyone who helped afterward?", type="yes_no"
        )
    ...
    def sexual_contact_questions(self):
        ...
        if contact_response and contact_response.lower() != "skip":
            ...
            if any(
                x in contact_response.lower() for x in ["digital", "finger", "hand"]
            ):
        ...
        ans = yield Question(
            "Did the assailant remove or tear any of your clothing?", type="yes_no"
        )
        self.interview.sexual_contact.clothing_removed_torn = self._parse_yes_no(ans)

        activities_response = yield Question(
    ...
    def injury_questions(self):
        ...
        assault_response = yield Question(
            "Did the person hit, slap, kick, bite, or strangle (choke) you? (list all that apply)"
        )
        ...
        symptoms_response = yield Question(
        ...
        ans = yield Question(
            "Are you having pain, discharge, or bleeding in your private parts (genital or anal areas)?"
        )
    ...
    def forensic_questions(self):
        ...
        ans = yield Question(
            "Have you peed (urinated) since the incident?", type="yes_no"
        )
        self.interview.forensic_evidence.urinated_since = self._parse_yes_no(ans)

        ans = yield Question(
            "Have you had a bowel movement (pooped) since the incident?", type="yes_no"
        )
        self.interview.forensic_evidence.defecated_since = self._parse_yes_no(ans)

        ans = yield Question(
            "Have you changed pads or tampons since the incident?", type="yes_no"
        )
        self.interview.forensic_evidence.changed_sanitary_products = self._parse_yes_no(
            ans
        )

        ans = yield Question(
            "Have you eaten, drunk, or smoked since the incident?", type="yes_no"
        )
        self.interview.forensic_evidence.eaten_drunk_smoked = self._parse_yes_no(ans)

        ans = yield Question(
            "Did the person involved leave any items behind (hair, condom, tissues, etc.)?",
            type="yes_no",
        )
        ...
        ans = yield Question(
            "Are you wearing the same clothes from the assault?", type="yes_no"
        )
        self.interview.forensic_evidence.wearing_same_clothes = self._parse_yes_no(ans)

        ans = yield Question(
            "Would you like these items collected for evidence?", type="yes_no"
        )
    ...
    def treatment_questions(self):
        ...
        ans = yield Question(
            "Would you like emergency contraception (if applicable)?", type="yes_no"
        )
    ...
    def psychological_questions(self):
        ...
        ans = yield Question(
            "Do you have someone you trust to talk to about this?", type="yes_no"
        )
        ...
        ans = yield Question(
            "Have you ever experienced anything like this before?", type="yes_no"
        )
        ...
        ans = yield Question(
            "Would you like to speak with a counselor or advocate?", type="yes_no"
        )
    ...
    def legal_questions(self):
        ...
        ans = yield Question(
            "Have you already reported this assault to the police?", type="yes_no"
        )
        self.interview.legal_followup.reported_to_police = self._parse_yes_no(ans)

        ans = yield Question(
            "Would you like me to explain how reporting works?", type="yes_no"
        )
        self.interview.legal_followup.wants_reporting_explanation = self._parse_yes_no(
            ans
        )
        ...
        ans = yield Question(
            "Can we contact you for follow-up medical results?", type="yes_no"
        )
        ...
        if self.interview.legal_followup.contact_for_followup == YesNoUnsure.YES:
            ans = yield Question(
                "What is the best way to contact you? (phone/email)", allow_skip=False
            )
            self.interview.legal_followup.contact_information = ans

        ans = yield Question(
            "Do you need help with transportation tonight?", type="yes_no"
        )
        self.interview.legal_followup.needs_transportation = self._parse_yes_no(ans)

        ans = yield Question(
            "Do you need help with safe housing tonight?", type="yes_no"
        )
