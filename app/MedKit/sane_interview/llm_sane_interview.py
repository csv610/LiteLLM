...
class LLMAssistedSANEInterviewer(SANEInterviewer):
    ...
    def get_response_with_llm_assist(
        ...
        if (
            response.lower() == "skip"
            or not self.use_llm_assist
            or not enable_suggestions
        ):
    ...
    def _generate_llm_suggestions(
        ...
        # INJURY ASSESSMENT suggestions
            ...
            if (
                "neck" in response_lower
                or "throat" in response_lower
                or "chok" in response_lower
            ):
                suggestions.append(
                    QuestionSuggestion(
        ...
        # SEXUAL CONTACT suggestions
            ...
            if "shower" in response_lower or "bath" in response_lower:
                if not any(
                    "how long" in q.lower() for q in context.questions_already_asked
                ):
                    suggestions.append(
                        QuestionSuggestion(
        ...
        # FORENSIC EVIDENCE suggestions
            ...
            if "clothes" in response_lower:
                if not any(
                    "which clothes" in q.lower()
                    for q in context.questions_already_asked
                ):
                    suggestions.append(
                        QuestionSuggestion(
        ...
        # PSYCHOLOGICAL suggestions
            ...
            if any(
                ...
                for word in ["scared", "afraid", "terrified", "anxious"]
            ):
                if not any(
                    "safe" in q.lower() for q in context.questions_already_asked
                ):
                    suggestions.append(
                        QuestionSuggestion(
            ...
            if any(
                word in response_lower
                for word in ["suicide", "hurt myself", "end it", "don't want to live"]
            ):
                suggestions.append(
                    QuestionSuggestion(
    ...
    def _handle_llm_suggestions(self, context: InterviewContext, section: str):
        ...
        self.io.display("─" * 70)
        self.io.display(
            "Review these suggestions and decide what to ask (if anything):\n"
        )
        ...
        for i, suggestion in enumerate(suggestions, 1):
            ...
            self.io.display(
                f"{priority_emoji} Suggestion {i} [{suggestion.priority.upper()} priority]:"
            )
        ...
        if choice == "s":
            ...
            if 0 <= idx < len(suggestions):
                ...
                self._store_followup_response(
                    section, selected.question, followup_response
                )
    ...
    def _store_followup_response(self, section: str, question: str, response: str):
        ...
        self.interview.additional_notes = (
            self.interview.additional_notes or ""
        ) + entry
        self.questions_asked.append(question)
    ...
    def incident_questions(self):
        ...
        self.interview.incident_history.incident_date = (
            self.get_response_with_llm_assist(
        ...
        self.interview.incident_history.incident_time = (
            self.get_response_with_llm_assist(
        ...
        num_response = self.get_response("How many individuals were involved? (number)")
        ...
        if self.interview.incident_history.weapons_used == YesNoUnsure.YES:
            self.interview.incident_history.weapon_details = (
                self.get_response_with_llm_assist(
        ...
        if self.interview.incident_history.physically_restrained == YesNoUnsure.YES:
            self.interview.incident_history.restraint_details = (
                self.get_response_with_llm_assist(
                    "How were you restrained?", section="incident_history"
                )
            )
        ...
        if self.interview.incident_history.forced_substances == YesNoUnsure.YES:
            self.interview.incident_history.substance_details = (
                self.get_response_with_llm_assist(
                    "What substances were involved?", section="incident_history"
                )
            )
        ...
        if self.interview.incident_history.witnesses == YesNoUnsure.YES:
            self.interview.incident_history.witness_details = (
                self.get_response_with_llm_assist(
    ...
    def injury_questions(self):
        ...
        if self.interview.injury_assessment.has_pain == YesNoUnsure.YES:
            self.interview.injury_assessment.pain_locations = (
                self.get_response_with_llm_assist(
                    "Where is the pain located?", section="injury_assessment"
                )
            )
    ...
    def sexual_contact_questions(self):
        ...
        if self.interview.sexual_contact.ejaculation_noted == YesNoUnsure.YES:
            self.interview.sexual_contact.ejaculation_location = (
                self.get_response_with_llm_assist(
                    "Where did ejaculation occur?", section="sexual_contact"
                )
            )
        ...
        if self.interview.sexual_contact.objects_used == YesNoUnsure.YES:
            self.interview.sexual_contact.object_details = (
                self.get_response_with_llm_assist(
                    "Can you describe the object(s)?", section="sexual_contact"
                )
            )
        ...
        if self.interview.sexual_contact.able_to_resist == YesNoUnsure.YES:
            self.interview.sexual_contact.resistance_details = (
                self.get_response_with_llm_assist(
                    "How did you resist?", section="sexual_contact"
                )
            )
    ...
    def psychological_questions(self):
        ...
        self.interview.psychological.current_emotional_state = (
            self.get_response_with_llm_assist(
                "How are you feeling emotionally right now?", section="psychological"
            )
        )
    ...
    def display_statistics(self):
        ...
        self.io.display("=" * 70)
        self.io.display(
            f"Total suggestions made: {self.llm_suggestions_accepted + self.llm_suggestions_rejected}"
        )
        ...
        if (self.llm_suggestions_accepted + self.llm_suggestions_rejected) > 0:
            acceptance_rate = (
                self.llm_suggestions_accepted
                / (self.llm_suggestions_accepted + self.llm_suggestions_rejected)
                * 100
            )
