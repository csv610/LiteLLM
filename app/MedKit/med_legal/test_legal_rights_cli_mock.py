...
def test_main_generate_success(mock_generator_class, mock_configure_logging, tmp_path):
    ...
    with patch.object(sys, "argv", test_args):
        ...
        mock_generator.generate_text.assert_called_once_with(
            topic="Test Topic", country="India", structured=False
        )
...
def test_main_generate_file_input(
    ...
    with patch.object(sys, "argv", test_args):
        ...
        mock_generator.generate_text.assert_any_call(
            topic="Topic A", country="India", structured=False
        )
        mock_generator.generate_text.assert_any_call(
            topic="Topic B", country="India", structured=False
        )
