...
def main():
    ...
    )  # Not required to allow 'list' as default or handle it manually

    # List command
    subparsers.add_parser("list", help="List all available article modules")
    ...
    search_p.add_argument(
        "-n", "--num", type=int, default=5, help="Number of articles to return"
    )

    # Review
    review_p = subparsers.add_parser("review", help="Review a medical article")
    review_p.add_argument(
        "-f", "--file", required=True, help="Path to the article file"
    )
    ...
    summarize_p.add_argument(
        "-m", "--model", default="ollama/gemma3", help="Model to use"
    )

    # Compare
    compare_p = subparsers.add_parser("compare", help="Compare two medical articles")
    compare_p.add_argument(
        "-f1", "--file1", required=True, help="Path to the first article"
    )
    compare_p.add_argument(
        "-f2", "--file2", required=True, help="Path to the second article"
    )
    compare_p.add_argument(
        "-m", "--model", default="ollama/gemma3", help="Model to use"
    )
    ...
    # Keywords
    keywords_p = subparsers.add_parser(
        "keywords", help="Extract keywords from a medical article"
    )
    keywords_p.add_argument(
        "-f", "--file", required=True, help="Path to the article file"
    )
    keywords_p.add_argument(
        "-m", "--model", default="ollama/gemma3", help="Model to use"
    ...
    if args.command == "list":
        ...
        from medkit_article.article_search.articles_search_cli import (
            main as search_main,
        )
