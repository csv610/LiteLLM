...
def test_cli_text_input(mock_extractor_class):
    ...
    with patch("article_keywords.cli.Path.mkdir"), patch(
        "article_keywords.cli.Path.parent", new=Path("/tmp/fake_parent")
    ):
...
def test_cli_file_input(mock_extractor_class, tmp_path):
    ...
    mock_extractor.load_results.return_value = []
    mock_extractor.extract_keywords.return_value = {"id": "1", "keywords": ["kw1"]}
