...
def test_generate_text_structured(mock_lite_client):
    ...
    mock_data = MedicalProcedureInfoModel(
        ...
        purpose=ProcedurePurpose(
            primary_purpose="Remove appendix",
            therapeutic_uses="Appendicitis",
            diagnostic_uses="None",
            preventive_uses="None",
        ),
        indications=ProcedureIndications(
            when_recommended="Inflamed appendix",
            symptoms_requiring_procedure="Abdominal pain",
            conditions_treated="Appendicitis",
            contraindications="None",
        ),
        preparation=PreparationRequirements(
            fasting_required="8 hours",
            medication_adjustments="None",
            dietary_restrictions="NPO",
            pre_procedure_tests="CBC, CT",
            items_to_bring="ID",
        ...
        details=ProcedureDetails(
            ...
            step_by_step_process="1. Incision",
            duration="1 hour",
            location="Hospital",
            equipment_used="Laparoscope",
            hospital_stay="1 day",
        ),
        risks=DiscomfortAndRisks(
            discomfort_level="Moderate",
            common_sensations="Grogginess",
            common_side_effects="Nausea",
            serious_risks="Infection",
            complication_rates="Low",
            mortality_risk="Very low",
        ),
        recovery=RecoveryInformation(
            immediate_recovery="Waking up",
        ...
        outcomes=OutcomesAndEffectiveness(
            success_rate="99%",
            expected_benefits="Cure",
            symptom_improvement="Immediate",
            long_term_outcomes="Good",
            factors_affecting_outcomes="Age",
        ),
        follow_up=FollowUpCare(
            follow_up_schedule="2 weeks",
            monitoring_required="Incision check",
            lifestyle_changes="None",
            medications_after="Antibiotics",
            physical_therapy="No",
        ),
        alternatives=Alternatives(
            alternative_procedures="Open surgery",
            non_surgical_alternatives="Antibiotics",
            advantages_over_alternatives="Faster recovery",
            when_alternatives_preferred="Rupture",
        ),
        technical=TechnicalDetails(
            surgical_approach="Laparoscopic",
            technology_used="Robot",
            procedure_variations="Single port",
        ...
        evidence=ProcedureEvidence(
            ...
            procedure_limitations=ProcedureLimitations(
                not_suitable_for="Unstable patients",
                age_limitations="None",
                medical_conditions_precluding="Coagulopathy",
                anatomical_limitations="Obesity",
            ),
        ),
