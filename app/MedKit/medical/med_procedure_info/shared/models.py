...
class MedicalAccuracy(BaseModel):
    ...
    inaccuracies: List[str] = Field(
        description="List of specific medical inaccuracies found, if any."
    )
    missing_critical_info: List[str] = Field(
        description="List of critical medical information that is missing."
    )


class SafetyCheck(BaseModel):
    score: int = Field(
        description="Score from 1-10 regarding patient safety information."
    )
    contraindications_check: str = Field(
        description="Evaluation of how well contraindications are covered."
    )
    risk_assessment: str = Field(
        description="Evaluation of how well risks and complications are described."
    )
...
class ClarityAndUsability(BaseModel):
    score: int = Field(
        description="Score from 1-10 regarding clarity and usability for the target audience."
    )
    jargon_usage: str = Field(
        description="Assessment of medical jargon usage - is it appropriate or too complex?"
    )
    structure_feedback: str = Field(
        description="Feedback on the structure and flow of the information."
    )


class EvidenceBasedReview(BaseModel):
    score: int = Field(
        description="Score from 1-10 regarding adherence to evidence-based practices."
    )
    outdated_practices: List[str] = Field(
        description="List of any outdated practices mentioned."
    )
    guideline_alignment: str = Field(
        description="How well the content aligns with standard medical guidelines."
    )
