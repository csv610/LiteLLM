...
def get_user_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate comprehensive medical procedure documentation.")
    parser.add_argument("procedure", help="Procedure name or file path containing names.")
...
def main():
    ...
    configure_logging(log_file="medical_procedure_info.log", verbosity=args.verbosity, enable_console=True)
    
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    input_path = Path(args.procedure)
    items = [line.strip() for line in open(input_path)] if input_path.is_file() else [args.procedure]
