"""
graphagents.py - Multi-agent implementation using LangGraph for med_procedure_info.
"""
...
class GraphMedProcedureInfoAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_procedure_info: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_procedure_info synthesize: {state['specialist_output']}")
