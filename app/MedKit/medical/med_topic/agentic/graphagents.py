"""
graphagents.py - Multi-agent implementation using LangGraph for med_topic.
"""
...
from app.MedKit.medical.med_topic.shared.models import ModelOutput
...
class GraphMedTopicAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_topic: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_topic synthesize: {state['specialist_output']}")
