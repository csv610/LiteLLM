...
def get_user_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("condition", help="Condition name or file path containing names.")
...
def main():
    ...
    configure_logging(log_file="synthetic_case_report.log", verbosity=args.verbosity, enable_console=True)
    
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    input_path = Path(args.condition)
    items = [line.strip() for line in open(input_path)] if input_path.is_file() else [args.condition]
