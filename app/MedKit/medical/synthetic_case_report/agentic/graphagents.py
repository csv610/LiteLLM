"""
graphagents.py - Multi-agent implementation using LangGraph for synthetic_case_report.
"""
...
class GraphSyntheticCaseReportAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for synthetic_case_report: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for synthetic_case_report synthesize: {state['specialist_output']}")
