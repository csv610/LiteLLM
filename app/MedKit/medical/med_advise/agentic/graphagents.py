"""
graphagents.py - Multi-agent implementation using LangGraph for med_advise.
"""
...
class GraphMedAdviseAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_advise: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_advise synthesize: {state['specialist_output']}")
