"""
agnoagents.py - Multi-agent implementation using Agno framework for med_advise.
"""
...
from app.MedKit.medical.med_advise.shared.models import ModelOutput
...
class AgnoMedAdviseAgents:
    def __init__(self, model_name: str = "gemini/gemini-2.5-flash"):
        self.model_name = model_name
        self.model = get_agno_model(model_name)
        
        self.specialist = Agent(
            name="med_advise Specialist",
            role="Expert in med_advise tasks",
            model=self.model,
            instructions=["Provide deep analysis and structured insights for med_advise."],
        )

        self.verifier = Agent(
            name="med_advise Verifier",
        ...
        self.manager = Agent(
            name="med_advise Manager",
            team=[self.specialist, self.verifier],
            model=self.model,
            instructions=[
                "Coordinate the med_advise workflow.",
