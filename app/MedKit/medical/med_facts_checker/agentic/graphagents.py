"""
graphagents.py - Multi-agent implementation using LangGraph for med_facts_checker.
"""
...
class GraphMedFactsCheckerAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_facts_checker: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_facts_checker synthesize: {state['specialist_output']}")
