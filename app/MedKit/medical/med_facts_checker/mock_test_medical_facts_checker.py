...
from medical.med_facts_checker.medical_facts_checker_models import (
    MedicalFactFictionAnalysisModel, ModelOutput, DetailedAnalysis,
    StatementAnalysis, ContextInformation, AnalyzerMetadata,
    FactualSupport, FictionIndicators
)

@pytest.fixture
def mock_lite_client():
    with patch('medical.med_facts_checker.medical_facts_checker.LiteClient') as mock:
        yield mock
...
def test_generate_text_unstructured(mock_lite_client):
    config = ModelConfig(model="test-model")
    checker = MedicalFactsChecker(config)
    mock_output = ModelOutput(markdown="Fact: Water is wet", data=None)
    mock_lite_client.return_value.generate_text.return_value = mock_output
    
    result = checker.generate_text("Water is wet")
    assert result.markdown == "Fact: Water is wet"
    assert checker.statement == "Water is wet"
...
@patch('medical.med_facts_checker.medical_facts_checker.save_model_response')
def test_save_success(mock_save, mock_lite_client):
    config = ModelConfig(model="test-model")
    checker = MedicalFactsChecker(config)
    mock_output = ModelOutput(markdown="Fact analysis")
    mock_lite_client.return_value.generate_text.return_value = mock_output
