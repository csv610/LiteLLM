...
class FollowUpQuestionModel(BaseModel):
    ...
    clinical_reason: str = Field(
        description="Why this follow-up is clinically important"
    )
    investigation_focus: str = Field(
        description="What specific diagnostic or clinical aspect is being investigated"
    )
...
class HistoryQuestionModel(BaseModel):
    ...
    clinical_relevance: str = Field(
        description="Why this question is clinically relevant"
    )
    requirement: QuestionRequirement = Field(
        description="Whether mandatory or optional"
    )
    expected_answer_type: str = Field(
        description="Type of answer expected (yes/no, descriptive, date, etc.)"
    )


class PastConditionQuestionModel(HistoryQuestionModel):
    condition_category: str = Field(
        description="Category of condition (cardiac, respiratory, endocrine, etc.)"
    )
...
class HospitalizationQuestionModel(HistoryQuestionModel):
    scope: str = Field(
        description="Scope of hospitalization question (frequency, reason, duration, etc.)"
    )
    follow_up_questions: List[FollowUpQuestionModel] = Field(default_factory=list)
...
class SurgeryQuestionModel(HistoryQuestionModel):
    detail_level: str = Field(
        description="Level of detail requested (list, specific procedures, complications)"
    )
    follow_up_questions: List[FollowUpQuestionModel] = Field(default_factory=list)


class FamilyHistoryQuestionModel(HistoryQuestionModel):
    family_member_type: str = Field(
        description="Type of family member (parent, sibling, grandparent, etc.)"
    )
...
class MedicationQuestionModel(HistoryQuestionModel):
    aspect: str = Field(
        description="Aspect of medication use (current medications, dosage, adherence, side effects)"
    )
    follow_up_questions: List[FollowUpQuestionModel] = Field(default_factory=list)
...
class AllergyQuestionModel(HistoryQuestionModel):
    allergy_type: str = Field(
        description="Type of allergy (medication, food, environmental, latex, contrast dye)"
    )
    detail_aspect: str = Field(
        description="Aspect of allergy to assess (type, reaction, severity, management)"
    )
    follow_up_questions: List[FollowUpQuestionModel] = Field(default_factory=list)
...
class LifestyleQuestionModel(HistoryQuestionModel):
    category: str = Field(
        description="Lifestyle category (tobacco, alcohol, diet, exercise, sleep, stress)"
    )
    detail_requested: str = Field(
        description="Type of detail requested (frequency, quantity, impact, duration)"
    )
    follow_up_questions: List[FollowUpQuestionModel] = Field(default_factory=list)


class PersonalSocialQuestionModel(HistoryQuestionModel):
    social_aspect: str = Field(
        description="Social aspect (occupation, housing, relationships, education, support systems)"
    )
    follow_up_questions: List[FollowUpQuestionModel] = Field(default_factory=list)
