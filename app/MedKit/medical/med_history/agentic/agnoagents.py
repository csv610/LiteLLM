"""
agnoagents.py - Multi-agent implementation using Agno framework for med_history.
"""
...
from app.MedKit.medical.med_history.shared.models import ModelOutput
...
class AgnoMedHistoryAgents:
    def __init__(self, model_name: str = "gemini/gemini-2.5-flash"):
        self.model_name = model_name
        self.model = get_agno_model(model_name)
        
        self.specialist = Agent(
            name="med_history Specialist",
            role="Expert in med_history tasks",
            model=self.model,
            instructions=["Provide deep analysis and structured insights for med_history."],
        )

        self.verifier = Agent(
            name="med_history Verifier",
        ...
        self.manager = Agent(
            name="med_history Manager",
            team=[self.specialist, self.verifier],
            model=self.model,
            instructions=[
                "Coordinate the med_history workflow.",
