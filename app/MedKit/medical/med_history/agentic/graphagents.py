"""
graphagents.py - Multi-agent implementation using LangGraph for med_history.
"""
...
class GraphMedHistoryAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_history: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_history synthesize: {state['specialist_output']}")
