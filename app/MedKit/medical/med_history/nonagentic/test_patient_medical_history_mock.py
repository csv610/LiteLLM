...
from medical.med_history.agentic.patient_medical_history_models import (
    DrugInformationQuestionsModel,
    FamilyHistoryQuestionsModel,
    FollowUpQuestionModel,
    LifestyleAndSocialQuestionsModel,
    ModelOutput,
    PastConditionQuestionModel,
    PastMedicalHistoryQuestionsModel,
    PatientMedicalHistoryModel,
    QuestionRequirement,
    VaccinationQuestionsModel,
)
...
def mock_lite_client():
    with patch("medical.med_history.agentic.patient_medical_history.LiteClient") as mock:
        yield mock
...
def test_generate_text_structured(mock_lite_client):
    ...
    mock_data = PatientMedicalHistoryModel(
            ...
            condition_questions=[
                PastConditionQuestionModel(
                    ...
                    follow_up_questions=[
                        FollowUpQuestionModel(
                            question="Since when?",
                            clinical_reason="Duration matters",
                            investigation_focus="Severity",
                        )
                    ],
                )
            ],
        ...
        drug_information=DrugInformationQuestionsModel(
            medication_questions=[], allergy_questions=[], adverse_reaction_questions=[]
        ),
        ...
        lifestyle_and_social=LifestyleAndSocialQuestionsModel(
            lifestyle_questions=[], personal_social_questions=[]
        ),
    )
...
@patch("medical.med_history.agentic.patient_medical_history.save_model_response")
