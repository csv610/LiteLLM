...
def get_user_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate comprehensive herbal information.")
...
def main():
    ...
    configure_logging(log_file="herbal_info.log", verbosity=args.verbosity, enable_console=True)
    
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    input_path = Path(args.herb)
    items = [line.strip() for line in open(input_path)] if input_path.is_file() else [args.herb]
