"""
graphagents.py - Multi-agent implementation using LangGraph for herbal_info.
"""
...
class GraphHerbalInfoAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for herbal_info: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for herbal_info synthesize: {state['specialist_output']}")
