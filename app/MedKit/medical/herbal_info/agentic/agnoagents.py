"""
agnoagents.py - Multi-agent implementation using Agno framework for herbal_info.
"""
...
from app.MedKit.medical.herbal_info.shared.models import ModelOutput
...
class AgnoHerbalInfoAgents:
    def __init__(self, model_name: str = "gemini/gemini-2.5-flash"):
        self.model_name = model_name
        self.model = get_agno_model(model_name)
        
        self.specialist = Agent(
            name="herbal_info Specialist",
            role="Expert in herbal_info tasks",
            model=self.model,
            instructions=["Provide deep analysis and structured insights for herbal_info."],
        )

        self.verifier = Agent(
            name="herbal_info Verifier",
        ...
        self.manager = Agent(
            name="herbal_info Manager",
            team=[self.specialist, self.verifier],
            model=self.model,
            instructions=[
                "Coordinate the herbal_info workflow.",
