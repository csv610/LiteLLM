"""
graphagents.py - Multi-agent implementation using LangGraph for med_speciality.
"""
...
class GraphMedSpecialityAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_speciality: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_speciality synthesize: {state['specialist_output']}")
