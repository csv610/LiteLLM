...
class SpecialtyCategory(BaseModel):
    ...
    name: str = Field(
        ...,
        description="Name of the category (e.g., 'Cardiovascular', 'Neurological', 'Digestive')",
    )
...
class Subspecialty(BaseModel):
    ...
    description: str = Field(
        ..., description="Brief description of what the subspecialty focuses on"
    )
...
class MedicalSpecialist(BaseModel):
    ...
    specialty_name: str = Field(
        ..., description="Official name of the medical specialty"
    )
    category: SpecialtyCategory = Field(
        ..., description="Category this specialty belongs to"
    )
    description: str = Field(
        ..., description="Detailed description of what this specialist does"
    )
    treats: List[str] = Field(
        ..., description="List of conditions, diseases, or body systems treated"
    )
    common_referral_reasons: List[str] = Field(
        ..., description="Common reasons patients see this specialist"
    )
    subspecialties: Optional[List[Subspecialty]] = Field(
        default=None, description="Subspecialties within this field"
    )
    is_surgical: bool = Field(
        default=False, description="Whether this is primarily a surgical specialty"
    )
    patient_population: Optional[str] = Field(
        default=None,
        description="Specific patient population if applicable (e.g., 'children', 'elderly', 'women')",
    )
...
class MedicalSpecialistDatabase(BaseModel):
    ...
    def get_by_category(self, category_name: str) -> List[MedicalSpecialist]:
        ...
        return [
            ...
            if s.category.name.lower() == category_name.lower()
        ]
