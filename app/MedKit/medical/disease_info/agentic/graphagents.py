"""
graphagents.py - Multi-agent implementation using LangGraph for disease_info.
"""
...
class GraphDiseaseInfoAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for disease_info: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for disease_info synthesize: {state['specialist_output']}")
