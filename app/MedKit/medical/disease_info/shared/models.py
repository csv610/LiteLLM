...
class RiskFactorsModel(BaseModel):
    ...
    modifiable: List[str] = Field(
        description="Risk factors that can be changed (e.g., smoking, diet, exercise)."
    )
    non_modifiable: List[str] = Field(
        description="Risk factors that cannot be changed (e.g., age, genetics, family history)."
    )
...
class DiagnosticCriteriaModel(BaseModel):
    ...
    laboratory_tests: List[str] = Field(
        description="Recommended laboratory tests (e.g., blood tests, urine tests)."
    )
    imaging_studies: List[str] = Field(
        description="Recommended imaging studies (e.g., X-ray, CT scan, MRI)."
    )
...
class DiseaseBackgroundModel(BaseModel):
    ...
    pathophysiology: str = Field(
        description="The underlying physiological process of the disease."
    )
...
class DiseaseEpidemiologyModel(BaseModel):
    ...
    prevalence: str = Field(
        description="The proportion of a population found to have the disease."
    )
    incidence: str = Field(
        description="The number of new cases of the disease during a certain period."
    )
    risk_factors: RiskFactorsModel = Field(
        description="Factors that increase the risk of developing the disease."
    )
...
class DiseaseClinicalPresentationModel(BaseModel):
    ...
    signs: List[str] = Field(
        description="Objective medical signs observed by a clinician."
    )
    natural_history: str = Field(
        description="The progression of the disease without treatment."
    )
...
class DiseaseDiagnosisModel(BaseModel):
    ...
    diagnostic_criteria: DiagnosticCriteriaModel = Field(
        description="Criteria used to establish a diagnosis."
    )
...
class DiseaseManagementModel(BaseModel):
    ...
    treatment_options: List[str] = Field(
        description="Available treatment options (e.g., medications, therapies)."
    )
...
class DiseaseResearchModel(BaseModel):
    ...
    recent_advancements: str = Field(
        description="Recent breakthroughs in diagnosis or treatment."
    )
...
class DiseaseLivingWithModel(BaseModel):
    ...
    quality_of_life: str = Field(
        description="Impact on quality of life and daily activities."
    )
    support_resources: List[str] = Field(
        description="Patient support groups and resources."
    )
...
class DiseaseInfoModel(BaseModel):
    ...
    background: DiseaseBackgroundModel = Field(
        description="Background and pathophysiology."
    )
    epidemiology: DiseaseEpidemiologyModel = Field(description="Epidemiological data.")
    clinical_presentation: DiseaseClinicalPresentationModel = Field(
        description="Clinical presentation."
    )
    diagnosis: DiseaseDiagnosisModel = Field(
        description="Diagnostic criteria and methods."
    )
    management: DiseaseManagementModel = Field(
        description="Treatment and management strategies."
    )
    research: DiseaseResearchModel = Field(
        description="Current research and advancements."
    )
    special_populations: DiseaseSpecialPopulationsModel = Field(
