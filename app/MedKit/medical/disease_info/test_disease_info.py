...
from medical.disease_info.disease_info_models import (
    DiseaseInfoModel, ModelOutput, DiseaseIdentityModel, DiseaseBackgroundModel, 
    DiseaseEpidemiologyModel, DiseaseClinicalPresentationModel, DiseaseDiagnosisModel, 
    DiseaseManagementModel, DiseaseResearchModel, DiseaseSpecialPopulationsModel,
    DiseaseLivingWithModel, RiskFactorsModel, DiagnosticCriteriaModel
)

@pytest.fixture
def mock_lite_client():
    with patch('medical.disease_info.disease_info.LiteClient') as mock:
        yield mock
...
def test_generate_text_unstructured(mock_lite_client):
    config = ModelConfig(model="test-model")
    generator = DiseaseInfoGenerator(config)
    mock_output = ModelOutput(markdown="Disease information in markdown", data_available=True)
...
def test_generate_text_structured(mock_lite_client):
    ...
    mock_data = DiseaseInfoModel(
        identity=DiseaseIdentityModel(name="Diabetes", disease_name="Diabetes", icd_10_code="E11", icd10_code="E11", icd11_code="5A11", synonyms=[]),
        background=DiseaseBackgroundModel(definition="Chronic", pathophysiology="Insulin", etiology="Multiple factors"),
        ...
        clinical_presentation=DiseaseClinicalPresentationModel(symptoms=[], complications=[], signs=[], natural_history="Progressive"),
