...
def test_download_images(mock_ddg_images, generator):
    ...
    downloaded = generator.download_images("test query", num_images=1, output_dir=output_dir)
    
    assert len(downloaded) == 1
    assert downloaded[0] == output_dir / 'image_1.jpg'
    mock_ddg_images_instance.get_urls.assert_called_with("test query", "Medium", 1)
    mock_ddg_images_instance.download_image.assert_called_with('http://example.com/1.jpg', str(output_dir), "test query", 0)

def test_search_videos(mock_ddg_videos, generator):
    mock_ddg_videos_instance = generator.video_searcher
    mock_ddg_videos_instance.get_urls.return_value = [{'url': 'http://video.com', 'title': 'Video 1'}]
...
def test_save(mock_save_model_response, generator):
    ...
    mock_save_model_response.assert_called_with(result, output_dir / "test_topic_analysis")
