"""
graphagents.py - Multi-agent implementation using LangGraph for med_media.
"""
...
class GraphMedMediaAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_media: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_media synthesize: {state['specialist_output']}")
