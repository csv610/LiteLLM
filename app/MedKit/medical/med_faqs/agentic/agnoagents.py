"""
agnoagents.py - Multi-agent implementation using Agno framework for med_faqs.
"""
...
from app.MedKit.medical.med_faqs.shared.models import ModelOutput
...
class AgnoMedFaqsAgents:
    def __init__(self, model_name: str = "gemini/gemini-2.5-flash"):
        self.model_name = model_name
        self.model = get_agno_model(model_name)
        
        self.specialist = Agent(
            name="med_faqs Specialist",
            role="Expert in med_faqs tasks",
            model=self.model,
            instructions=["Provide deep analysis and structured insights for med_faqs."],
        )

        self.verifier = Agent(
            name="med_faqs Verifier",
        ...
        self.manager = Agent(
            name="med_faqs Manager",
            team=[self.specialist, self.verifier],
            model=self.model,
            instructions=[
                "Coordinate the med_faqs workflow.",
