"""
graphagents.py - Multi-agent implementation using LangGraph for med_faqs.
"""
...
class GraphMedFaqsAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_faqs: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_faqs synthesize: {state['specialist_output']}")
