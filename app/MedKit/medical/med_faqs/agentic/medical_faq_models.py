...
class MisconceptionItemModel(BaseModel):
    ...
    explanation: str = Field(
        description="Why the misconception exists and how to address it"
    )
...
class WhenToSeekCareModel(BaseModel):
    ...
    urgency_level: str = Field(
        description="Urgency: immediate, urgent, soon, or routine"
    )
...
class SeeAlsoTopicsModel(BaseModel):
    ...
    category: str = Field(
        description="Category of reference: 'topic', 'test', or 'device'"
    )
    description: str = Field(
        description="Brief description of what this reference covers"
    )
    relevance: str = Field(
        description="Why this cross-reference is relevant to the current topic"
    )
...
class ProviderFAQModel(BaseModel):
    ...
    topic_name: str = Field(description="Medical topic name")
    clinical_overview: str = Field(
        description="Clinical overview for healthcare providers"
    )
    clinical_faqs: List[FAQItemModel] = Field(
        description="Clinically-focused Q&A pairs"
    )
    evidence_based_practices: List[str] = Field(
        description="Current best practices and evidence"
    )
    quality_metrics: List[str] = Field(
        description="Outcome measures and quality indicators"
    )
