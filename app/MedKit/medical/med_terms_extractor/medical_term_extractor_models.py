...
class MedicalTermsModel(BaseModel):
    ...
    diseases: List[Disease] = Field(
        default_factory=list, description="Diseases and conditions found"
    )
    medicines: List[Medicine] = Field(
        default_factory=list, description="Medicines and drugs found"
    )
    symptoms: List[Symptom] = Field(
        default_factory=list, description="Symptoms and signs found"
    )
    treatments: List[Treatment] = Field(
        default_factory=list, description="Treatments found"
    )
    procedures: List[Procedure] = Field(
        default_factory=list, description="Procedures and tests found"
    )
    specialties: List[Specialty] = Field(
        default_factory=list, description="Medical specialties found"
    )
    anatomical_terms: List[AnatomicalTerm] = Field(
        default_factory=list, description="Anatomical structures found"
    )
    side_effects: List[SideEffect] = Field(
        default_factory=list, description="Side effects and adverse reactions found"
    )
