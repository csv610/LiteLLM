...
def get_user_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate comprehensive medical anatomy information.")
    parser.add_argument("body_part", help="Anatomical part or file path containing parts.")
...
def main():
    ...
    configure_logging(log_file="medical_anatomy.log", verbosity=args.verbosity, enable_console=True)
    
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    input_path = Path(args.body_part)
    items = [line.strip() for line in open(input_path)] if input_path.is_file() else [args.body_part]
