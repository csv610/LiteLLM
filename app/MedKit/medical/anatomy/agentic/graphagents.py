"""
graphagents.py - Multi-agent implementation using LangGraph for anatomy.
"""
...
class GraphAnatomyAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for anatomy: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for anatomy synthesize: {state['specialist_output']}")
