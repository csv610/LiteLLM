"""
graphagents.py - Multi-agent implementation using LangGraph for surgical_tool_info.
"""
...
from app.MedKit.medical.surgical_tool_info.shared.models import ModelOutput
...
class GraphSurgicalToolInfoAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for surgical_tool_info: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for surgical_tool_info synthesize: {state['specialist_output']}")
