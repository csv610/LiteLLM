...
from organ_disease_info_models import OrganDiseasesModel

@pytest.fixture
def mock_lite_client():
    with patch('organ_disease_info.LiteClient') as mock:
        yield mock
...
def test_generate_text_unstructured(mock_lite_client):
    ...
    mock_client_instance.generate_text.return_value = "Organ diseases in markdown"
    
    output = generator.generate_text(organ="Heart", structured=False)
    
    assert output.markdown == "Organ diseases in markdown"
...
def test_generate_text_structured(mock_lite_client):
    ...
    mock_data = MagicMock(spec=OrganDiseasesModel)
    
    mock_client_instance = mock_lite_client.return_value
    mock_client_instance.generate_text.return_value = mock_data
    
    output = generator.generate_text(organ="Heart", structured=True)
