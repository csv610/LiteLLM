"""
graphagents.py - Multi-agent implementation using LangGraph for surgical_tray.
"""
...
class GraphSurgicalTrayAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for surgical_tray: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for surgical_tray synthesize: {state['specialist_output']}")
