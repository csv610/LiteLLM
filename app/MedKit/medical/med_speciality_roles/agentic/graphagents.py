"""
graphagents.py - Multi-agent implementation using LangGraph for med_speciality_roles.
"""
...
class GraphMedSpecialityRolesAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_speciality_roles: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_speciality_roles synthesize: {state['specialist_output']}")
