...
class SpecialityRoleInfo(BaseModel):
    ...
    primary_focus: str = Field(
        description="The body systems or conditions the speciality focuses on."
    )
    key_responsibilities: List[str] = Field(
        description="What the specialist generally does (diagnose, treat, manage)."
    )
    common_procedures: List[str] = Field(
        description="Examples of procedures or tests they perform."
    )
...
class MedicalSpecialityRolesModel(BaseModel):
    ...
    roles_info: SpecialityRoleInfo = Field(
        description="Roles and responsibilities info"
    )
