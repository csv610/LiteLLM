"""
agnoagents.py - Multi-agent implementation using Agno framework for med_quiz.
"""
...
from app.MedKit.medical.med_quiz.shared.models import ModelOutput
...
class AgnoMedQuizAgents:
    def __init__(self, model_name: str = "gemini/gemini-2.5-flash"):
        self.model_name = model_name
        self.model = get_agno_model(model_name)
        
        self.specialist = Agent(
            name="med_quiz Specialist",
            role="Expert in med_quiz tasks",
            model=self.model,
            instructions=["Provide deep analysis and structured insights for med_quiz."],
        )

        self.verifier = Agent(
            name="med_quiz Verifier",
        ...
        self.manager = Agent(
            name="med_quiz Manager",
            team=[self.specialist, self.verifier],
            model=self.model,
            instructions=[
                "Coordinate the med_quiz workflow.",
