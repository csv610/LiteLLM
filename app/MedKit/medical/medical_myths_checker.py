        ...
        try:
            ...
            for i, statement in enumerate(tqdm(statements, desc="Analyzing myths", unit="myth"), 1):
                try:
                    ...
                    if not isinstance(response, MythAnalysisResponse):
                        self.logger.error(f"Expected MythAnalysisResponse for statement {i}, got {type(response)}")
                        failed_myths.append({"statement": statement, "error": f"Invalid response type: {type(response)}"})
                        continue

                    # Extract myths from the MythAnalysisResponse object
                    for myth in response.myths:
                        myth_dict = myth.model_dump()
                        self.myths_data.append(myth_dict)
                        # Save immediately to file if output file is specified
                        if self.output_file:
                            self._append_to_file(myth_dict)
