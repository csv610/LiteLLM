"""
graphagents.py - Multi-agent implementation using LangGraph for med_myths_checker.
"""
...
from app.MedKit.medical.med_myths_checker.shared.models import ModelOutput
...
class GraphMedMythsCheckerAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_myths_checker: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_myths_checker synthesize: {state['specialist_output']}")
