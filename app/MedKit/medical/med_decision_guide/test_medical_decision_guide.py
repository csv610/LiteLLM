...
from medical.med_decision_guide.medical_decision_guide_models import (
    MedicalDecisionGuideModel, ModelOutput, DecisionNode, Outcome
)

@pytest.fixture
def mock_lite_client():
    with patch('medical.med_decision_guide.medical_decision_guide.LiteClient') as mock:
        yield mock
...
def test_generate_text_structured(mock_lite_client):
    ...
    mock_data = MedicalDecisionGuideModel(
        ...
        start_node_id="q1",
        decision_nodes=[
            DecisionNode(node_id="q1", question="Is it crushing pain?", yes_node_id="out_emergency", no_node_id="q2"),
            DecisionNode(node_id="q2", question="Is it sharp?", yes_node_id="out_mild", no_node_id="out_moderate")
        ],
        outcomes=[
            Outcome(outcome_id="out_emergency", severity_level="Emergency", urgency="Emergency", recommendation="Call 911", possible_diagnoses="MI", home_care_advice="None", warning_signs="Crushing pain"),
            Outcome(outcome_id="out_mild", severity_level="Mild", urgency="Self-care", recommendation="Rest", possible_diagnoses="Pleurisy", home_care_advice="OTC pain relief", warning_signs="Worsening pain"),
            Outcome(outcome_id="out_moderate", severity_level="Moderate", urgency="Urgent-care", recommendation="See doctor", possible_diagnoses="GERD", home_care_advice="Antacids", warning_signs="Pain after eating")
        ],
...
@patch('medical.med_decision_guide.medical_decision_guide.save_model_response')
