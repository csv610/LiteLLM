"""
graphagents.py - Multi-agent implementation using LangGraph for med_decision_guide.
"""
...
class GraphMedDecisionGuideAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for med_decision_guide: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for med_decision_guide synthesize: {state['specialist_output']}")
