...
class DecisionNode(BaseModel):
    ...
    uncertain_node_id: Optional[str] = Field(
        default=None,
        description="Node ID to go to if answer is uncertain (optional - if present, creates third branch)",
    )
...
class Outcome(BaseModel):
    ...
    severity_level: str = Field(
        description="Assessed severity (mild, moderate, severe, emergency)"
    )
    urgency: str = Field(
        description="Care urgency (self-care, urgent-care, emergency, specialist)"
    )
    recommendation: str = Field(description="Clinical recommendation and next steps")
    possible_diagnoses: str = Field(
        description="Possible diagnoses to consider, comma-separated"
    )
    home_care_advice: str = Field(
        description="Home management strategies if appropriate"
    )
    warning_signs: str = Field(
        description="Red flags requiring immediate attention, comma-separated"
    )
