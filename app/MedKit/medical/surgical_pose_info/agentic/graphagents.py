"""
graphagents.py - Multi-agent implementation using LangGraph for surgical_pose_info.
"""
...
class GraphSurgicalPoseInfoAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for surgical_pose_info: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for surgical_pose_info synthesize: {state['specialist_output']}")
