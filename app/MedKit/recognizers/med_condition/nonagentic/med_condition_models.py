...
class MedicalConditionIdentificationModel(BaseModel):
    ...
    condition_name: str = Field(
        description="The name of the medical condition being identified"
    )
    is_well_known: bool = Field(
        description="Whether the condition is widely recognized in healthcare"
    )
    category: str = Field(
        description="The category of the condition (e.g., chronic, acute, genetic, injury)"
    )
    key_characteristics: List[str] = Field(
        description="List of primary signs, symptoms, or features of this condition"
    )
    clinical_significance: str = Field(
        description="Explanation of its importance in clinical practice and patient care"
    )
...
class MedicalConditionIdentifierModel(BaseModel):
    ...
    summary: str = Field(
        description="A concise summary of the condition's status in healthcare"
    )
    data_available: bool = Field(
        description="Whether information about this condition was found"
    )
