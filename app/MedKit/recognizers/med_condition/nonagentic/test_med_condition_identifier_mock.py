#!/usr/bin/env python3
"""
Test suite for MedicalConditionIdentifier Module.
...
from app.MedKit.recognizers.med_condition.nonagentic.med_condition_identifier import (
    MedicalConditionIdentifier,
)
from app.MedKit.recognizers.med_condition.nonagentic.med_condition_models import (
    MedicalConditionIdentificationModel,
    MedicalConditionIdentifierModel,
    ModelOutput,
)
from app.MedKit.recognizers.med_condition.nonagentic.med_condition_prompts import (
...
def test_models():
    ...
    identification = MedicalConditionIdentificationModel(
        ...
        is_well_known=True,
        category="chronic",
        key_characteristics=["elevated blood pressure", "often asymptomatic"],
        clinical_significance="Major risk factor for cardiovascular disease",
    )
    print("✓ IdentificationModel instantiated successfully")

    identifier_model = MedicalConditionIdentifierModel(
        identification=identification,
        summary="Hypertension is a recognized medical condition in healthcare",
        data_available=True,
    )
