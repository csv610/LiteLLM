...
class MedicalDeviceIdentificationModel(BaseModel):
    ...
    device_name: str = Field(
        description="The name of the medical device being identified"
    )
    is_well_known: bool = Field(
        description="Whether the device is widely recognized in healthcare"
    )
    device_category: str = Field(
        description="The category of device (e.g., implantable, diagnostic, surgical, wearable)"
    )
    primary_function: str = Field(description="What the device is primarily used for")
    clinical_significance: str = Field(
        description="Explanation of its importance in patient care or medical procedures"
    )
...
class MedicalDeviceIdentifierModel(BaseModel):
    ...
    summary: str = Field(
        description="A concise summary of the device's status in healthcare"
    )
    data_available: bool = Field(
        description="Whether information about this device was found"
    )
