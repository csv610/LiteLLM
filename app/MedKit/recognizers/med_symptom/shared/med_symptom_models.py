...
class MedicalSymptomIdentificationModel(BaseModel):
    ...
    symptom_name: str = Field(
        description="The name of the medical symptom being identified"
    )
    is_well_known: bool = Field(
        description="Whether the symptom is widely recognized in clinical practice"
    )
    associated_conditions: List[str] = Field(
        description="Common medical conditions or diseases associated with this symptom"
    )
    severity_indicators: str = Field(
        description="Signs that this symptom might indicate a medical emergency or serious condition"
    )
    clinical_description: str = Field(
        description="A brief medical description of how the symptom manifests"
    )
...
class MedicalSymptomIdentifierModel(BaseModel):
    ...
    summary: str = Field(
        description="A concise summary of the symptom's clinical relevance"
    )
    data_available: bool = Field(
        description="Whether information about this symptom was found"
    )
