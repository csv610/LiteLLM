...
class VaccineIdentificationModel(BaseModel):
    ...
    vaccine_name: str = Field(
        description="The name of the vaccine (e.g., MMR, BCG, Pfizer-BioNTech COVID-19)"
    )
    is_well_known: bool = Field(
        description="Whether the vaccine is widely recognized in public health"
    )
    target_diseases: List[str] = Field(
        description="Diseases the vaccine is designed to prevent"
    )
    vaccine_type: str = Field(
        description="Type of vaccine (e.g., mRNA, Live-attenuated, Inactivated, Subunit)"
    )
    standard_schedule: str = Field(
        description="Typical administration schedule or age group"
    )
