...
class MedicalAnatomyIdentificationModel(BaseModel):
    ...
    structure_name: str = Field(
        description="The name of the anatomical structure being identified"
    )
    is_well_known: bool = Field(
        description="Whether the structure is widely recognized in anatomy and medicine"
    )
    system: str = Field(
        description="The primary body system it belongs to (e.g., cardiovascular, nervous, skeletal)"
    )
    location: str = Field(description="General anatomical location in the body")
    clinical_significance: str = Field(
        description="Explanation of its importance in clinical practice, surgery, or pathology"
    )
...
class MedicalAnatomyIdentifierModel(BaseModel):
    ...
    summary: str = Field(
        description="A concise summary of the structure's status in anatomy"
    )
    data_available: bool = Field(
        description="Whether information about this structure was found"
    )
