...
class AbbreviationIdentificationModel(BaseModel):
    ...
    is_well_known: bool = Field(
        description="Whether the abbreviation is standard in medical practice"
    )
    context_of_use: str = Field(
        description="Typical context where this is used (e.g., prescriptions, physical exams)"
    )
    clinical_meaning: str = Field(
        description="A brief explanation of what the term represents clinically"
    )
