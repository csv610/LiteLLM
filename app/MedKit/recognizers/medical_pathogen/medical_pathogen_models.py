...
class PathogenIdentificationModel(BaseModel):
    ...
    pathogen_name: str = Field(
        description="The scientific or common name of the pathogen"
    )
    is_well_known: bool = Field(
        description="Whether the pathogen is widely recognized in medicine"
    )
    pathogen_type: str = Field(
        description="Type of pathogen (e.g., bacteria, virus, fungus, parasite)"
    )
    associated_infections: List[str] = Field(
        description="Infections or diseases caused by this pathogen"
    )
    clinical_significance: str = Field(
        description="Brief explanation of its impact on health and treatment approach"
    )
