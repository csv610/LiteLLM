...
class DrugIdentificationModel(BaseModel):
    ...
    is_well_known: bool = Field(
        description="Whether the drug is widely recognized in the pharmaceutical industry"
    )
    common_uses: List[str] = Field(
        description="List of primary medical conditions this drug is used to treat"
    )
    regulatory_status: str = Field(
        description="Brief overview of its approval status (e.g., FDA approved, EMA approved)"
    )
    industry_significance: str = Field(
        description="Explanation of why the drug is or isn't considered well-known"
    )
...
class DrugIdentifierModel(BaseModel):
    ...
    summary: str = Field(
        description="A concise summary of the drug's status in the industry"
    )
