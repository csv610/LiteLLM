...
class MedicationClassIdentificationModel(BaseModel):
    ...
    class_name: str = Field(
        description="The name of the medication class (e.g., SSRIs, Statins)"
    )
    is_well_known: bool = Field(
        description="Whether the class is widely recognized in pharmacology"
    )
    mechanism_of_action: str = Field(
        description="How medications in this class typically work in the body"
    )
    common_examples: List[str] = Field(
        description="Common drug names that belong to this class"
    )
    therapeutic_uses: List[str] = Field(
        description="Primary medical conditions treated by this class"
    )
