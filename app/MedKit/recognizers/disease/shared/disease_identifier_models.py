...
class DiseaseIdentificationModel(BaseModel):
    ...
    is_well_known: bool = Field(
        description="Whether the disease is widely recognized in the medical community"
    )
    common_symptoms: List[str] = Field(
        description="List of primary symptoms associated with this disease"
    )
    prevalence: str = Field(
        description="General information about how common the disease is (e.g., rare, common, epidemic)"
    )
    medical_significance: str = Field(
        description="Explanation of its impact on public health or clinical practice"
    )
...
class DiseaseIdentifierModel(BaseModel):
    ...
    summary: str = Field(
        description="A concise summary of the disease's status in medicine"
    )
    data_available: bool = Field(
        description="Whether information about this disease was found"
    )
