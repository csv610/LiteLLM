...
class SupplementIdentificationModel(BaseModel):
    ...
    supplement_name: str = Field(
        description="The name of the supplement (e.g., Vitamin D3, Ashwagandha)"
    )
    is_well_known: bool = Field(
        description="Whether the supplement is widely recognized in health and wellness"
    )
    primary_nutrients: List[str] = Field(
        description="Key active ingredients or nutrients"
    )
    common_uses: List[str] = Field(
        description="Primary reasons people take this supplement"
    )
    regulatory_standing: str = Field(
        description="Brief overview of its regulatory status (e.g., FDA regulated as food)"
    )
...
class SupplementIdentifierModel(BaseModel):
    ...
    data_available: bool = Field(
        description="Whether information about this supplement was found"
    )
