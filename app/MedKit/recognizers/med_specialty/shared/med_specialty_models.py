...
class MedicalSpecialtyIdentificationModel(BaseModel):
    ...
    specialty_name: str = Field(
        description="The name of the medical specialty being identified"
    )
    is_well_known: bool = Field(
        description="Whether the specialty is widely recognized in medicine"
    )
    organs_treated: List[str] = Field(
        description="Major organs or body systems this specialty focuses on"
    )
    common_procedures: List[str] = Field(
        description="Common medical or surgical procedures performed by specialists in this field"
    )
    clinical_scope: str = Field(
        description="A brief description of the types of conditions and patients this specialty manages"
    )
...
class MedicalSpecialtyIdentifierModel(BaseModel):
    ...
    summary: str = Field(
        description="A concise summary of the specialty's role in healthcare"
    )
    data_available: bool = Field(
        description="Whether information about this specialty was found"
    )
