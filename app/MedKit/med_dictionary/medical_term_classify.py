...
from med_dictionary.medical_classification_prompts import (
    MedicalClassificationPromptBuilder,
)
...
class MedicalTermClassifier:
    ...
    def __init__(self, model_config: ModelConfig, output_file: Path = None):
        ...
        self.output_file = output_file or (
            Path(__file__).parent / "outputs" / "classified.json"
        )
    ...
    def save(self) -> bool:
        ...
        try:
            sorted_data = sorted(
                self.classifications, key=lambda x: x.get("term", "").lower()
            )
    ...
    def classify(self, input_data: str) -> int:
        ...
        new_terms = sorted(
            [term for term in terms if term.lower() not in self.existing_terms]
        )
        if not new_terms:
        ...
        for term in tqdm(new_terms, desc="Processing terms"):
            try:
                ...
                if raw_response and raw_response.strip():
                    ...
                    if (
                        classification_data
                        and "category" in classification_data
                        and "subcategory" in classification_data
                    ):
                        ...
                        logger.error(
                            f"Invalid classification data for '{term}': {raw_response}"
                        )
            except Exception as e:
        ...
        self.save()
        logger.info(
            f"Completed: {saved_count}/{len(new_terms)} classified. Total: {len(self.classifications)}. Results saved to: {self.output_file}"
        )
        return saved_count
...
def main():
    parser = argparse.ArgumentParser(
        description="Classify medical terms into categories and subcategories."
    )
    parser.add_argument(
