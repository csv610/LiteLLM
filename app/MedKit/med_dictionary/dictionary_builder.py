...
def load_terms_from_file(file_path: Path) -> List[str]:
    ...
    try:
        if file_path.suffix == ".txt":
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                for row in reader:
                    if row:
                        terms.append(
                            row[0].strip()
                        )  # Assume first column contains the term
            ...
            logger.warning(
                f"Unsupported file type for loading terms: {file_path.suffix}"
            )
...
class DictionaryBuilder:
    ...
    def __init__(self, model_config: ModelConfig):
        ...
        self.output_file = (
            Path(__file__).parent / "outputs" / f"dictionary_{safe_model}.json"
        )
    ...
    def save(self) -> bool:
        ...
        try:
            sorted_defs = sorted(
                self.definitions, key=lambda x: x.get("term", "").lower()
            )
    ...
    def generate_text(self, input_data: str) -> int:
        ...
        if temp_definitions:
            ...
            if self.save():
                print(
                    f"Merged {len(temp_definitions)} new terms into the main dictionary."
                )
            else:
