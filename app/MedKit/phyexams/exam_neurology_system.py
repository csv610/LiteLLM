...
class PatientAnswer(BaseModel):
    question_id: str = Field(
        ..., description="Unique identifier for the patient question"
    )
    question_text: str = Field(
        ..., description="Full text of the question asked to the patient"
    )
    answer_text: str = Field(..., description="Patient's free-text response")
    answer_code: Optional[str] = Field(
        None, description="Structured code representing the type of answer"
    )
    confidence: Optional[float] = Field(
        None,
        ge=0.0,
        le=1.0,
        description="Confidence score for the answer classification",
    )
...
class NeurologyExam(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.now, description="Timestamp of assessment"
    )
    patient_answers: Optional[List[PatientAnswer]] = Field(
        None, description="Patient answers to neurology questions"
    )
    nurse_report: NurseReport = Field(
        ..., description="Detailed neurology examination findings from the nurse"
    )
    llm_assessment: Optional[LLMAssessment] = Field(
        None, description="AI-powered assessment and recommendations"
    )
