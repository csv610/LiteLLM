...
class BreastQuestion(BaseModel):
    ...
    question_text: str = Field(
        description="The breast examination question to ask the patient"
    )
    assessment_domain: str = Field(
        description="Domain being assessed (e.g., pain, lumps, discharge)"
    )
...
class BreastTestQuestions(BaseModel):
    ...
    questions: List[BreastQuestion] = Field(
        description="List of breast assessment questions"
    )
    assessment_context: str = Field(
        description="Clinical context for breast assessment"
    )
...
class PatientSymptoms(BaseModel):
    ...
    nipple_discharge: str = Field(
        description="Any nipple discharge (color, consistency, unilateral/bilateral)"
    )
    skin_changes: str = Field(
        description="Any skin changes on breasts (dimpling, redness, rash)"
    )
    family_history: str = Field(
        description="Any family history of breast or ovarian cancer"
    )
    axillary_symptoms: str = Field(
        description="Any lumps, swelling, or tenderness in armpits"
    )
    recent_trauma: str = Field(
        description="Any recent trauma or injury to breasts or axillae"
    )
    hormonal_changes: str = Field(
        description="Any hormonal symptoms (cyclical pain, swelling)"
    )
...
class PhysicalExamination(BaseModel):
    ...
    breast_inspection: str = Field(
        description="Breast inspection findings (asymmetry, skin changes, nipple changes)"
    )
    breast_palpation: str = Field(
        description="Breast tissue palpation findings (masses, nodules, tenderness)"
    )
    breast_quadrant_assessment: str = Field(
        description="Assessment of all four quadrants of breast tissue"
    )
    nipple_assessment: str = Field(
        description="Nipple assessment (retraction, discharge, symmetry)"
    )
    skin_assessment: str = Field(
        description="Skin assessment (color, texture, rashes, dimpling)"
    )
    axillary_palpation: str = Field(
        description="Axillary palpation findings (masses, tenderness)"
    )
    axillary_lymph_nodes: str = Field(
        description="Axillary lymph node assessment (size, consistency, mobility)"
    )
    supraclavicular_lymph_nodes: str = Field(
        description="Supraclavicular lymph node assessment"
    )
    chest_wall_assessment: str = Field(
        description="Chest wall assessment (masses, tenderness)"
    )
...
class RiskFactors(BaseModel):
    ...
    menarche_age: str = Field(
        description="Age at menarche (early menarche increases risk)"
    )
    menopause_status: str = Field(description="Menopausal status and age at menopause")
    pregnancy_history: str = Field(
        description="Number of pregnancies and age at first pregnancy"
    )
    breastfeeding_history: str = Field(
        description="Duration of breastfeeding (protective factor)"
    )
    hormone_therapy: str = Field(
        description="Current or past hormone replacement therapy use"
    )
...
class BreastAssessmentSummary(BaseModel):
    ...
    breast_status: str = Field(
        description="Overall breast status - normal/benign/suspicious/abnormal"
    )
    symptom_summary: str = Field(description="Summary of reported symptoms")
    findings_significance: str = Field(
        description="Clinical significance of findings - none/benign/concerning/urgent"
    )
    masses_identified: str = Field(
        description="Any masses identified - none/benign/suspicious/malignant features"
    )
    lymph_node_status: str = Field(
        description="Lymph node assessment - normal/enlarged/concerning"
    )
    risk_level: str = Field(
        description="Breast cancer risk level - low/moderate/high/very high"
    )
    imaging_recommendations: str = Field(
        description="Recommended imaging (mammography, ultrasound, MRI, etc.)"
    )
    specialist_referral: str = Field(
        description="Whether oncology or breast surgery referral recommended"
    )
    follow_up_plan: str = Field(description="Recommended follow-up plan and timeline")
    patient_education: str = Field(
        description="Patient education provided regarding breast health and self-exam"
    )
...
def ask_breast_questions(
    ...
    print()
    print(
        "MEASURES: This assessment evaluates breast health, identifies potential abnormalities,"
    )
    ...
    if not any(k.startswith("q") for k in responses.keys()):
        ...
        responses["lumps_masses"] = input(
            "Any lumps or masses noticed in the breasts?: "
        ).strip()
        responses["nipple_discharge"] = input(
            "Any nipple discharge? If yes, color and consistency?: "
        ).strip()
        responses["skin_changes"] = input(
            "Any skin changes (dimpling, redness, rash)?: "
        ).strip()
        responses["family_history"] = input(
            "Any family history of breast or ovarian cancer?: "
        ).strip()
        responses["axillary_symptoms"] = input(
            "Any lumps, swelling, or tenderness in armpits?: "
        ).strip()
        responses["recent_trauma"] = input(
            "Any recent trauma or injury to breasts or axillae?: "
        ).strip()
        responses["hormonal_changes"] = input(
            "Any hormonal symptoms (cyclical pain, swelling)?: "
        ).strip()
    ...
    responses["breast_inspection"] = input(
        "Breast inspection findings (asymmetry, skin changes, nipple): "
    ).strip()
    responses["breast_palpation"] = input(
        "Breast tissue palpation findings (masses, nodules, tenderness): "
    ).strip()
    responses["breast_quadrants"] = input(
        "Assessment of all four quadrants of breast tissue: "
    ).strip()
    responses["nipple_assessment"] = input(
        "Nipple assessment (retraction, discharge, symmetry): "
    ).strip()
    responses["skin_assessment"] = input(
        "Skin assessment (color, texture, rashes, dimpling): "
    ).strip()
    responses["axillary_palpation"] = input(
        "Axillary palpation findings (masses, tenderness): "
    ).strip()
    responses["axillary_lymph_nodes"] = input(
        "Axillary lymph node assessment (size, consistency, mobility): "
    ).strip()
    responses["supraclavicular_lymph_nodes"] = input(
        "Supraclavicular lymph node assessment: "
    ).strip()
    responses["chest_wall"] = input(
        "Chest wall assessment (masses, tenderness): "
    ).strip()
...
def create_breast_assessment_from_responses(
    ...
    assessment_data = {
        ...
        "physical_examination": {
            ...
            "supraclavicular_lymph_nodes": responses.get(
    ...
    if output_path is None:
        output_path = (
            Path("outputs")
            / f"{patient_name.lower().replace(' ', '_')}_breast_axillae.json"
        )
...
def evaluate_breast_axillae(
    ...
    assessment = create_breast_assessment_from_responses(
