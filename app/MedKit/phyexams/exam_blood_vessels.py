...
class VascularQuestion(BaseModel):
    ...
    question_text: str = Field(
        description="The vascular examination question to ask the patient"
    )
    assessment_domain: str = Field(
        description="Domain being assessed (e.g., pain, swelling, pulses)"
    )
...
class VascularTestQuestions(BaseModel):
    ...
    questions: List[VascularQuestion] = Field(
        description="List of vascular assessment questions"
    )
    assessment_context: str = Field(
        description="Clinical context for vascular assessment"
    )
...
class PatientSymptoms(BaseModel):
    ...
    limb_pain: str = Field(
        description="Any limb pain, cramping, or claudication with walking"
    )
    ...
    color_changes: str = Field(
        description="Any color changes in skin: pale, cyanotic, red"
    )
    ...
    thrombosis_history: str = Field(
        description="History of deep vein thrombosis or pulmonary embolism"
    )
    risk_factors: str = Field(
        description="Any hypertension, diabetes, hyperlipidemia, smoking"
    )
...
class PhysicalExamination(BaseModel):
    ...
    blood_pressure: str = Field(
        description="Blood pressure measurements (systolic/diastolic)"
    )
    radial_pulses: str = Field(
        description="Radial pulse assessment (strength, symmetry)"
    )
    ...
    posterior_tibial_pulses: str = Field(
        description="Posterior tibial pulse assessment"
    )
    skin_inspection: str = Field(description="Skin color, temperature, edema, ulcers")
    carotid_auscultation: str = Field(
        description="Carotid artery auscultation for bruits"
    )
    femoral_auscultation: str = Field(
        description="Femoral artery auscultation for bruits"
    )
    ankle_brachial_index: str = Field(description="Ankle-brachial index if measured")
    extremity_measurements: str = Field(
        description="Limb circumference measurements if taken"
    )
...
class VascularRiskFactors(BaseModel):
    ...
    hypertension: str = Field(
        description="History of hypertension - none/controlled/uncontrolled"
    )
    diabetes: str = Field(
        description="History of diabetes - none/type 1/type 2/controlled/uncontrolled"
    )
    hyperlipidemia: str = Field(
        description="History of high cholesterol - none/yes/controlled/uncontrolled"
    )
    smoking_status: str = Field(
        description="Smoking status - never/former/current/quantity"
    )
    family_history: str = Field(
        description="Family history of vascular disease - none/yes/specific details"
    )
    obesity: str = Field(description="BMI/obesity status - normal/overweight/obese")
    physical_activity: str = Field(
        description="Physical activity level - sedentary/light/moderate/vigorous"
    )
    antiplatelet_therapy: str = Field(
        description="Use of antiplatelet medications - none/aspirin/clopidogrel/other"
    )
...
class VascularAssessmentSummary(BaseModel):
    ...
    vascular_status: str = Field(
        description="Overall vascular status - normal/low-risk/moderate-risk/high-risk"
    )
    symptom_severity: str = Field(
        description="Severity of vascular symptoms - none/mild/moderate/severe"
    )
    pulse_assessment: str = Field(
        description="Overall pulse assessment - normal/diminished/asymmetric/absent"
    )
    skin_findings: str = Field(
        description="Significant skin findings - none/edema/color changes/ulcers"
    )
    identified_risks: str = Field(
        description="Identified vascular risk factors, comma-separated"
    )
    acute_concerns: str = Field(
        description="Any acute vascular concerns - none/possible/probable/definite"
    )
    imaging_recommendations: str = Field(
        description="Recommended imaging (Doppler, CT, angiography, etc.)"
    )
    specialist_referral: str = Field(
        description="Whether cardiology or vascular surgery referral recommended"
    )
    follow_up_plan: str = Field(description="Recommended follow-up plan and timeline")
    patient_education: str = Field(
        description="Patient education provided regarding risk factors"
    )
...
def ask_vascular_questions(
    ...
    print()
    print(
        "MEASURES: This assessment evaluates the patient's peripheral vascular health,"
    )
    ...
    if client:
        ...
        try:
            vascular_qs = generate_vascular_questions(
    ...
    if not any(k.startswith("q") for k in responses.keys()):
        ...
        responses["limb_pain"] = input(
            "Any limb pain, cramping, or claudication with walking?: "
        ).strip()
        ...
        responses["color_changes"] = input(
            "Any color changes in skin (pale, cyanotic, red)?: "
        ).strip()
        responses["varicosities"] = input(
            "Any visible varicose veins or bulging veins?: "
        ).strip()
        responses["ulcers"] = input(
            "Any ulcers or non-healing wounds on legs or feet?: "
        ).strip()
        responses["thrombosis_history"] = input(
            "History of deep vein thrombosis or pulmonary embolism?: "
        ).strip()
        responses["risk_factors"] = input(
            "Any hypertension, diabetes, hyperlipidemia, or smoking?: "
        ).strip()
    ...
    responses["posterior_tibial"] = input(
        "Posterior tibial pulses assessment: "
    ).strip()
    responses["skin_inspection"] = input(
        "Skin inspection findings (color, temperature, edema, ulcers): "
    ).strip()
    responses["carotid_auscultation"] = input(
        "Carotid artery auscultation for bruits: "
    ).strip()
    responses["femoral_auscultation"] = input(
        "Femoral artery auscultation for bruits: "
    ).strip()
    responses["ankle_brachial_index"] = input(
        "Ankle-brachial index if measured: "
    ).strip()
    responses["extremity_measurements"] = input(
        "Limb circumference measurements if taken: "
    ).strip()
    ...
    responses["hypertension"] = input(
        "History of hypertension (none/controlled/uncontrolled): "
    ).strip()
    responses["diabetes"] = input(
        "History of diabetes (none/type 1/type 2/status): "
    ).strip()
    responses["hyperlipidemia"] = input(
        "History of high cholesterol (none/yes/status): "
    ).strip()
    responses["smoking_status"] = input(
        "Smoking status (never/former/current/quantity): "
    ).strip()
    ...
    responses["antiplatelet_therapy"] = input(
...
def create_vascular_assessment_from_responses(
    ...
    if output_path is None:
        output_path = (
            Path("outputs") / f"{patient_name.lower().replace(' ', '_')}_vascular.json"
        )
...
def evaluate_vascular_system(
    ...
    assessment = create_vascular_assessment_from_responses(
...
def main() -> int:
    """Main entry point for vascular system assessment."""
    ...
    try:
        ...
        print(f"✗ Error: {e}")
        logger.error("Error during vascular system assessment", exc_info=True)
        return 1
