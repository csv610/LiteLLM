...
class SimpleAddition(BaseModel):
    ...
    single_digit_addition: str = Field(
        description="Ability to add single-digit numbers (e.g., 3 + 5, 7 + 8). Provide patient's answers and accuracy"
    )
    two_digit_addition: str = Field(
        description="Ability to add two-digit numbers (e.g., 23 + 14, 45 + 27). Provide patient's answers and accuracy"
    )
    three_digit_addition: str = Field(
        description="Ability to add three-digit numbers (e.g., 125 + 234, 456 + 312). Provide patient's answers and accuracy"
    )
    multiple_additions: str = Field(
        description="Ability to add multiple numbers in sequence (e.g., 5 + 3 + 7, 12 + 8 + 15). Provide patient's answers and accuracy"
    )
    addition_speed: str = Field(
        description="Speed of response for addition problems - immediate/hesitant/delayed"
    )
    addition_errors: str = Field(
        description="Types of errors made in addition - none/occasional/frequent/systematic"
    )
...
class SimpleSubtraction(BaseModel):
    ...
    single_digit_subtraction: str = Field(
        description="Ability to subtract single-digit numbers (e.g., 8 - 3, 9 - 5). Provide patient's answers and accuracy"
    )
    two_digit_subtraction: str = Field(
        description="Ability to subtract two-digit numbers (e.g., 45 - 12, 78 - 34). Provide patient's answers and accuracy"
    )
    three_digit_subtraction: str = Field(
        description="Ability to subtract three-digit numbers (e.g., 456 - 123, 789 - 234). Provide patient's answers and accuracy"
    )
    subtraction_with_borrowing: str = Field(
        description="Ability to subtract when borrowing is required (e.g., 25 - 18, 100 - 45). Provide patient's answers and accuracy"
    )
    subtraction_speed: str = Field(
        description="Speed of response for subtraction problems - immediate/hesitant/delayed"
    )
    subtraction_errors: str = Field(
        description="Types of errors made in subtraction - none/occasional/frequent/systematic"
    )
...
class SimpleMultiplication(BaseModel):
    ...
    single_digit_multiplication: str = Field(
        description="Ability to multiply single-digit numbers (e.g., 3 × 4, 6 × 7, 8 × 9). Provide patient's answers and accuracy"
    )
    multiplication_by_ten: str = Field(
        description="Ability to multiply by 10, 100 (e.g., 7 × 10, 12 × 100). Provide patient's answers and accuracy"
    )
    two_digit_multiplication: str = Field(
        description="Ability to multiply two-digit numbers (e.g., 12 × 5, 15 × 3). Provide patient's answers and accuracy"
    )
    times_tables_recall: str = Field(
        description="Recall of basic times tables (2x, 5x, 10x tables). Describe fluency and accuracy"
    )
    multiplication_speed: str = Field(
        description="Speed of response for multiplication problems - immediate/hesitant/delayed"
    )
    multiplication_errors: str = Field(
        description="Types of errors made in multiplication - none/occasional/frequent/systematic"
    )
...
class SimpleDivision(BaseModel):
    ...
    single_digit_division: str = Field(
        description="Ability to divide single-digit results (e.g., 8 ÷ 2, 15 ÷ 3, 20 ÷ 4). Provide patient's answers and accuracy"
    )
    division_by_ten: str = Field(
        description="Ability to divide by 10, 100 (e.g., 50 ÷ 10, 200 ÷ 100). Provide patient's answers and accuracy"
    )
    division_with_remainder: str = Field(
        description="Ability to divide with remainders (e.g., 17 ÷ 5, 23 ÷ 4). Provide patient's answers and accuracy"
    )
    simple_fractions: str = Field(
        description="Understanding of simple fractions (e.g., half of 10, quarter of 20). Provide patient's answers and accuracy"
    )
    division_speed: str = Field(
        description="Speed of response for division problems - immediate/hesitant/delayed"
    )
    division_errors: str = Field(
        description="Types of errors made in division - none/occasional/frequent/systematic"
    )
...
class MixedOperations(BaseModel):
    ...
    order_of_operations: str = Field(
        description="Understanding of order of operations (e.g., 2 + 3 × 4, 10 - 2 × 3). Provide patient's answers and accuracy"
    )
    two_step_problems: str = Field(
        description="Ability to solve two-step problems (e.g., 'add 5 and 3, then multiply by 2'). Provide patient's answers and accuracy"
    )
    word_problems_simple: str = Field(
        description="Ability to solve simple word problems mentally (e.g., 'If you have 10 apples and eat 3, how many remain?'). Provide patient's answers and accuracy"
    )
    percentage_simple: str = Field(
        description="Ability to calculate simple percentages (e.g., 50% of 20, 10% of 100). Provide patient's answers and accuracy"
    )
    mixed_operations_speed: str = Field(
        description="Speed of response for mixed operation problems - immediate/hesitant/delayed"
    )
    mixed_operations_errors: str = Field(
        description="Types of errors made in mixed operations - none/occasional/frequent/systematic"
    )
...
class CalculationStrategies(BaseModel):
    ...
    counting_on_fingers: str = Field(
        description="Whether patient uses fingers for counting - not at all/occasionally/frequently/always"
    )
    counting_aloud: str = Field(
        description="Whether patient counts aloud or subvocalizes - not at all/occasionally/frequently/always"
    )
    calculation_strategies: str = Field(
        description="Observed strategies used (e.g., rounding, breaking into parts, starting from larger number, etc.)"
    )
    strategy_efficiency: str = Field(
        description="Effectiveness of strategies used - efficient/moderately efficient/inefficient/disorganized"
    )
    self_correction: str = Field(
        description="Ability to recognize and correct errors - excellent/good/fair/poor"
    )
    confidence_level: str = Field(
        description="Patient's confidence in their answers - high/moderate/low/varies by difficulty"
    )
...
class CognitiveLimitations(BaseModel):
    ...
    dyscalculia_indicators: str = Field(
        description="Indicators of dyscalculia or arithmetic disorder - present/absent/possible"
    )
    working_memory_limitations: str = Field(
        description="Evidence of working memory difficulties affecting calculation - yes/no/unclear"
    )
    attention_difficulties: str = Field(
        description="Difficulties maintaining attention during calculations - yes/no/minimal"
    )
    anxiety_during_calculation: str = Field(
        description="Math anxiety or performance anxiety observed - yes/no/mild/severe"
    )
    specific_operation_deficits: str = Field(
        description="Specific operations that are particularly difficult - none/addition/subtraction/multiplication/division/mixed"
    )
    processing_speed: str = Field(
        description="Overall processing speed for arithmetic - normal/slow/very slow/fast"
    )
...
class ContextualFactors(BaseModel):
    ...
    educational_background: str = Field(
        description="Patient's educational level and math instruction background"
    )
    language_barriers: str = Field(
        description="Any language barriers affecting understanding of problems - yes/no/possible"
    )
    cultural_numeracy_practices: str = Field(
        description="Cultural background and typical numeracy practices relevant to assessment"
    )
    fatigue_or_stress_impact: str = Field(
        description="Observable impact of fatigue, stress, or emotion on performance"
    )
    medication_or_health_effects: str = Field(
        description="Potential impact of medications, sleep deprivation, or health conditions"
    )
    motivation_level: str = Field(
        description="Patient's effort and motivation during assessment - high/moderate/low"
    )
...
class AssessmentSummary(BaseModel):
    ...
    calculation_ability_level: str = Field(
        description="Overall arithmetic calculation ability level (normal/mildly impaired/moderately impaired/severely impaired)"
    )
    arithmetic_strengths: str = Field(
        description="Identified arithmetic strengths by operation type, comma-separated"
    )
    arithmetic_weaknesses: str = Field(
        description="Identified arithmetic weaknesses by operation type, comma-separated"
    )
    processing_capabilities: str = Field(
        description="Assessment of mental processing speed and working memory for arithmetic"
    )
    clinical_significance: str = Field(
        description="Clinical significance of findings and relationship to diagnosis or condition"
    )
    recommendations: str = Field(
        description="Recommendations for cognitive support, remediation, or accommodations, comma-separated"
    )
    neuropsychological_referral: str = Field(
        description="Whether neuropsychological or educational evaluation is recommended and rationale"
    )
...
def ask_arithmetic_questions() -> dict:
    ...
    print(
        "  8. Can you solve simple word problems mentally (e.g., apples, percentages)?"
    )
    print(
        "  9. What strategies do you use for mental math (counting, rounding, breaking down)?"
    )
    print(
        " 10. Do you experience math anxiety, and how confident are you in your answers?"
    )
    ...
    responses["div_remainder"] = input(
        "What is 17 ÷ 5? (with remainder if needed) "
    ).strip()
    ...
    responses["word_1"] = input(
        "If you have 10 apples and eat 3, how many remain? "
    ).strip()
    responses["percent_1"] = input("What is 50% of 20? ").strip()

    # SELF-ASSESSMENT
    print("\n--- SELF-ASSESSMENT ---")
    responses["confidence"] = input(
        "How confident are you in your answers? (very/somewhat/not very): "
    ).strip()
    responses["strategy"] = input(
        "What strategies did you use? (counting, rounding, breaking down, etc.): "
    ).strip()
    responses["finger_use"] = input(
        "Did you use your fingers for counting? (no/yes/sometimes): "
    ).strip()
    responses["math_anxiety"] = input(
        "Do you feel anxious about math? (no/mild/moderate/severe): "
    ).strip()

    return responses


def create_arithmetic_assessment_from_responses(
    ...
    if output_path is None:
        output_path = (
            Path("outputs")
            / f"{patient_name.lower().replace(' ', '_')}_arithmetic_calculation.json"
        )
...
def evaluate_arithmetic_calculation(
    ...
    assessment = create_arithmetic_assessment_from_responses(
