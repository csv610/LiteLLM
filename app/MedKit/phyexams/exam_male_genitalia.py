...
class NurseReport(BaseModel):
    ...
    palpation: dict = Field(
        ..., description="Palpation: testicular size, consistency, tenderness, masses"
    )
    hernia_exam: dict = Field(
        ..., description="Inguinal hernia check, reducibility, tenderness"
    )
    urethral_exam: dict = Field(
        ..., description="Urethral meatus inspection, discharge, lesions"
    )
...
class LLMAssessment(BaseModel):
    primary_impression: str = Field(
        ..., description="Primary impression from LLM or clinical reasoning"
    )
...
class MaleGenitaliaExam(BaseModel):
    ...
    patient_answers: Optional[List[PatientAnswer]] = Field(
        None, description="Patient answers to genital health questions"
    )
    nurse_report: NurseReport = Field(..., description="Nurse examination findings")
    llm_assessment: Optional[LLMAssessment] = Field(
        None, description="AI-generated assessment"
    )
