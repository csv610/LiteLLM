...
class NurseReport(BaseModel):
    inspection: dict = Field(
        ..., description="Inspection of vulva, perineum, and surrounding areas"
    )
    palpation: dict = Field(
        ...,
        description="Palpation: vaginal walls, cervix, uterus, adnexa for masses, tenderness",
    )
    speculum_exam: dict = Field(
        ..., description="Speculum findings: cervix, discharge, lesions, bleeding"
    )
...
patient_questions = [
    ...
    (
        "q3_bleeding",
        "Any intermenstrual bleeding, post-coital bleeding, or heavy periods?",
    ),
    ("q4_urinary", "Any urinary symptoms like burning, frequency, or urgency?"),
    (
        "q5_sexual",
        "Any sexual discomfort, pain with intercourse, or changes in libido?",
    ),
    ("q6_trauma", "Any recent trauma or injury to genital area?"),
    (
        "q7_infection_history",
        "Any history of STIs, urinary, or gynecological infections?",
    ),
...
def llm_analyze_answer(question: str, answer: str) -> dict:
    ...
    if (
        "yes" in answer.lower()
        or "pain" in answer.lower()
        or "discharge" in answer.lower()
    ):
        return {
            "answer_code": "present",
            "confidence": 0.95,
            "follow_up": [
                f"Please describe severity, duration, and location of {question.lower()}."
            ],
        }
        ...
        return {
            "answer_code": "unclear",
            "confidence": 0.80,
            "follow_up": [f"Can you clarify your answer regarding {question.lower()}?"],
        }
...
def generate_llm_assessment_fge():
    return LLMAssessment(
        ...
        urgency="monitor",
        recommendations=[
            "Consider vaginal swab for culture",
