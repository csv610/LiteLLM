...
class MoodAssessment(BaseModel):
    ...
    current_mood: str = Field(
        description="Patient's current emotional mood state (happy, sad, neutral, anxious, angry, etc.)"
    )
    mood_frequency: str = Field(
        description="How frequently mood changes occur during the day"
    )
    mood_intensity: str = Field(
        description="Intensity of mood on a scale and how long episodes last"
    )
    mood_triggers: str = Field(
        description="Identified triggers for mood changes or emotional episodes, comma-separated"
    )
...
class StressAndAnxiety(BaseModel):
    ...
    anxiety_symptoms: str = Field(
        description="Physical and mental anxiety symptoms experienced, comma-separated"
    )
    anxiety_frequency: str = Field(
        description="How often anxiety episodes occur and their duration"
    )
    stress_management_techniques: str = Field(
        description="Current coping mechanisms used for stress, comma-separated"
    )
...
class EmotionalRegulation(BaseModel):
    ...
    emotion_control: str = Field(
        description="Patient's ability to control emotional responses"
    )
    impulse_control: str = Field(
        description="Ability to manage impulsive emotional reactions"
    )
    emotional_responses: str = Field(
        description="Typical emotional responses to challenging situations"
    )
    recovery_time: str = Field(
        description="Time required to recover from emotional episodes"
    )
...
class SocialAndRelationships(BaseModel):
    ...
    social_engagement: str = Field(
        description="Level of social interaction and engagement with others"
    )
    relationship_satisfaction: str = Field(
        description="Quality and stability of personal relationships"
    )
    social_support_system: str = Field(
        description="Availability of emotional support from family/friends, comma-separated"
    )
    isolation_feelings: str = Field(
        description="Feelings of loneliness or social isolation"
    )
...
class SleepAndEnergy(BaseModel):
    ...
    sleep_quality: str = Field(
        description="Quality of sleep reported by patient (insomnia, oversleeping, interrupted, etc.)"
    )
    ...
    fatigue_patterns: str = Field(
        description="Patterns of fatigue or lack of motivation"
    )
...
class CopingMechanisms(BaseModel):
    ...
    positive_coping: str = Field(
        description="Healthy coping mechanisms used, comma-separated"
    )
    negative_coping: str = Field(
        description="Unhealthy or avoidant coping patterns identified, comma-separated"
    )
    resilience_factors: str = Field(
        description="Factors that contribute to emotional resilience, comma-separated"
    )
    problem_solving_ability: str = Field(
        description="Patient's ability to address problems constructively"
    )
...
class MentalHealthHistory(BaseModel):
    ...
    past_episodes: str = Field(
        description="History of depression, anxiety, or other mental health episodes"
    )
    treatment_history: str = Field(
        description="Previous mental health treatments or therapies received"
    )
    family_history: str = Field(
        description="Family history of mental health conditions, comma-separated"
    )
    current_medications: str = Field(
        description="Current medications affecting mood or mental health, comma-separated"
    )
...
class LifeEvents(BaseModel):
    ...
    recent_stressors: str = Field(
        description="Recent major life stressors or changes, comma-separated"
    )
    positive_events: str = Field(
        description="Recent positive life events, comma-separated"
    )
    grief_or_loss: str = Field(
        description="Experience of loss or grief and current processing stage"
    )
    major_life_changes: str = Field(
        description="Recent significant life transitions, comma-separated"
    )
...
class SelfPerception(BaseModel):
    ...
    self_esteem: str = Field(
        description="Overall self-esteem and self-worth assessment"
    )
    self_confidence: str = Field(
        description="Level of confidence in abilities and decision-making"
    )
    self_criticism: str = Field(
        description="Level of self-criticism or negative self-talk patterns"
    )
    personal_goals: str = Field(
        description="Current personal goals and sense of purpose, comma-separated"
    )
...
class AssessmentSummary(BaseModel):
    ...
    stability_level: str = Field(
        description="Overall emotional stability rating (stable, moderate, concerning)"
    )
    key_strengths: str = Field(
        description="Key emotional strengths identified, comma-separated"
    )
    areas_of_concern: str = Field(
        description="Areas requiring attention or support, comma-separated"
    )
    recommendations: str = Field(
        description="Recommendations for improving emotional stability, comma-separated"
    )
    professional_referral: str = Field(
        description="Whether professional mental health referral is recommended and rationale"
    )
...
def ask_patient_questions() -> dict:
    ...
    print(
        "  3. What are the identified triggers for your mood changes or emotional episodes?"
    )
    print("  4. How would you rate your current stress level (1-10)?")
    print(
        "  5. Do you experience anxiety symptoms? If yes, describe them (physical/mental)."
    )
    print(
        "  6. How well can you control your emotional responses in challenging situations?"
    )
    print(
        "  7. Do you have a reliable support system and close relationships to rely on?"
    )
    print("  8. How would you describe your sleep quality and typical sleep duration?")
    print(
        "  9. What healthy coping strategies do you use to manage stress and emotions?"
    )
    print(
        " 10. Have you experienced any significant mental health episodes or life stressors?"
    )
    ...
    # MOOD ASSESSMENT
    print("\n--- MOOD ASSESSMENT ---")
    responses["current_mood"] = input(
        "How would you describe your current mood? (e.g., happy, sad, anxious, neutral): "
    ).strip()
    responses["mood_frequency"] = input(
        "How often do your moods change during the day? "
    ).strip()
    responses["mood_intensity"] = input(
        "On a scale of 1-10, how intense are your mood changes? How long do they last? "
    ).strip()
    responses["mood_triggers"] = input(
        "What triggers your mood changes? (comma-separated): "
    ).strip()
    ...
    responses["anxiety_symptoms"] = input(
        "Do you experience anxiety symptoms? If yes, describe them (physical/mental): "
    ).strip()
    responses["anxiety_frequency"] = input(
        "How often do you experience anxiety and how long do episodes last? "
    ).strip()
    responses["stress_management"] = input(
        "What techniques do you use to manage stress? (comma-separated): "
    ).strip()

    # EMOTIONAL REGULATION
    print("\n--- EMOTIONAL REGULATION ---")
    responses["emotion_control"] = input(
        "How well can you control your emotional responses? "
    ).strip()
    responses["impulse_control"] = input(
        "How well do you manage impulsive emotional reactions? "
    ).strip()
    responses["emotional_responses"] = input(
        "How do you typically respond to challenging situations? "
    ).strip()
    responses["recovery_time"] = input(
        "How long does it typically take you to recover from emotional episodes? "
    ).strip()

    # SOCIAL AND RELATIONSHIPS
    print("\n--- SOCIAL AND RELATIONSHIPS ---")
    responses["social_engagement"] = input(
        "How often do you engage socially with others? "
    ).strip()
    responses["relationship_quality"] = input(
        "How satisfied are you with your close relationships? "
    ).strip()
    responses["social_support"] = input(
        "Do you have people you can rely on for support? Who? "
    ).strip()
    responses["loneliness"] = input(
        "Do you feel lonely or isolated? How often? "
    ).strip()
    ...
    responses["sleep_duration"] = input(
        "How many hours of sleep do you typically get per night? "
    ).strip()
    responses["energy_levels"] = input(
        "How would you describe your daily energy levels? "
    ).strip()
    responses["fatigue_patterns"] = input(
        "Do you experience fatigue or lack of motivation? When? "
    ).strip()

    # COPING MECHANISMS
    print("\n--- COPING MECHANISMS ---")
    responses["positive_coping"] = input(
        "What healthy coping strategies do you use? (comma-separated): "
    ).strip()
    responses["negative_coping"] = input(
        "Do you use any unhealthy coping patterns? (comma-separated): "
    ).strip()
    responses["resilience_factors"] = input(
        "What factors help you stay resilient? (comma-separated): "
    ).strip()
    responses["problem_solving"] = input(
        "How do you typically address problems? "
    ).strip()

    # MENTAL HEALTH HISTORY
    print("\n--- MENTAL HEALTH HISTORY ---")
    responses["past_episodes"] = input(
        "Have you ever experienced depression, anxiety, or other mental health issues? "
    ).strip()
    responses["treatment_history"] = input(
        "Have you received any mental health treatment or therapy? "
    ).strip()
    responses["family_history"] = input(
        "Does mental health issues run in your family? (comma-separated): "
    ).strip()
    responses["current_medications"] = input(
        "Are you currently on any medications affecting mood? "
    ).strip()

    # LIFE EVENTS
    print("\n--- LIFE EVENTS ---")
    responses["recent_stressors"] = input(
        "What recent major life stressors or changes have you experienced? (comma-separated): "
    ).strip()
    responses["positive_events"] = input(
        "What recent positive events have you experienced? (comma-separated): "
    ).strip()
    responses["grief_or_loss"] = input(
        "Have you experienced loss or grief? How are you processing it? "
    ).strip()
    responses["life_changes"] = input(
        "What significant life transitions are you going through? (comma-separated): "
    ).strip()

    # SELF PERCEPTION
    print("\n--- SELF PERCEPTION ---")
    responses["self_esteem"] = input("How would you rate your self-esteem? ").strip()
    responses["self_confidence"] = input(
        "How confident are you in your abilities? "
    ).strip()
    responses["self_criticism"] = input(
        "How self-critical are you? Do you engage in negative self-talk? "
    ).strip()
    responses["personal_goals"] = input(
        "What are your current personal goals and sense of purpose? (comma-separated): "
    ).strip()

    return responses


def create_assessment_from_responses(
    ...
    if output_path is None:
        output_path = (
            Path("outputs")
            / f"{patient_name.lower().replace(' ', '_')}_emotional_stability.json"
        )
