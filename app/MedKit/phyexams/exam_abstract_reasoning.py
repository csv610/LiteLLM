...
class ProverbInterpretation(BaseModel):
    ...
    stitch_in_time_meaning: str = Field(
        description="Patient's explanation of 'A stitch in time saves nine' - what it means and its relevance to life"
    )
    stitch_in_time_application: str = Field(
        description="Example of how this proverb applies to real-life situations"
    )
    bird_in_hand_meaning: str = Field(
        description="Patient's explanation of 'A bird in the hand is worth two in the bush' - its meaning and significance"
    )
    bird_in_hand_application: str = Field(
        description="Example of how this proverb applies to decision-making or priorities"
    )
    rolling_stone_meaning: str = Field(
        description="Patient's explanation of 'A rolling stone gathers no moss' - what it means and interpretations"
    )
    rolling_stone_application: str = Field(
        description="Example or perspective on how this proverb relates to life experience or personal values"
    )
...
class FableComprehension(BaseModel):
    ...
    moral_extraction: str = Field(
        description="Patient's ability to identify the moral or lesson from a fable"
    )
    character_analysis: str = Field(
        description="Patient's understanding of character motivations and actions in fables"
    )
    symbolic_thinking: str = Field(
        description="Ability to identify symbolic elements and their meanings in stories"
    )
    abstract_application: str = Field(
        description="Ability to apply fable lessons to abstract or general life principles"
    )
...
class MetaphorAndAnalogy(BaseModel):
    ...
    metaphor_understanding: str = Field(
        description="Ability to interpret and explain metaphorical language"
    )
    analogy_reasoning: str = Field(
        description="Ability to understand relationships in analogies and complete patterns (e.g., 'A is to B as C is to ___')"
    )
    figure_of_speech_comprehension: str = Field(
        description="Understanding of idioms, figures of speech, and non-literal language"
    )
    symbol_interpretation: str = Field(
        description="Ability to interpret symbols and their abstract meanings in different contexts"
    )
...
class ConceptualThinking(BaseModel):
    ...
    concept_relationships: str = Field(
        description="Ability to identify relationships between abstract concepts"
    )
    pattern_recognition: str = Field(
        description="Ability to recognize patterns, similarities, and differences in abstract ideas"
    )
    generalization_ability: str = Field(
        description="Ability to generalize from specific examples to broader abstract principles"
    )
    hypothesis_formation: str = Field(
        description="Ability to form and reason about hypothetical or abstract scenarios"
    )
...
class SymbolicProcessing(BaseModel):
    ...
    symbolic_language: str = Field(
        description="Comfort and ability with symbolic representations (e.g., math symbols, metaphors)"
    )
    abstract_problem_solving: str = Field(
        description="Ability to solve problems that require abstract or symbolic reasoning"
    )
    representation_switching: str = Field(
        description="Ability to switch between different representations of the same concept"
    )
    logical_reasoning: str = Field(
        description="Ability to follow abstract logical reasoning and deduction"
    )
...
class CognitiveLimitations(BaseModel):
    ...
    concrete_thinking_tendency: str = Field(
        description="Tendency toward literal or concrete interpretation rather than abstract"
    )
    abstraction_difficulty: str = Field(
        description="Specific areas where abstract reasoning becomes challenging"
    )
    reasoning_flexibility: str = Field(
        description="Ability to shift perspectives or consider multiple interpretations"
    )
    conceptual_gaps: str = Field(
        description="Identified gaps or limitations in abstract conceptual understanding"
    )
...
class CulturalAndContextualFactors(BaseModel):
    ...
    cultural_proverb_familiarity: str = Field(
        description="Familiarity with proverbs and idioms from patient's cultural background"
    )
    language_considerations: str = Field(
        description="Any language barriers or differences affecting abstract reasoning assessment"
    )
    educational_background: str = Field(
        description="Patient's educational level and how it relates to abstract reasoning capabilities"
    )
    contextual_understanding: str = Field(
        description="Ability to understand how context influences meaning and interpretation"
    )
...
class AssessmentSummary(BaseModel):
    ...
    abstract_reasoning_level: str = Field(
        description="Overall abstract reasoning capability level (high, moderate, limited, impaired)"
    )
    cognitive_strengths: str = Field(
        description="Identified cognitive strengths in abstract reasoning, comma-separated"
    )
    areas_of_difficulty: str = Field(
        description="Areas where abstract reasoning is impaired or limited, comma-separated"
    )
    ...
    recommendations: str = Field(
        description="Recommendations for cognitive support, therapy, or further assessment, comma-separated"
    )
    neuropsychological_referral: str = Field(
        description="Whether neuropsychological evaluation is recommended and rationale"
    )
...
def ask_abstract_reasoning_questions() -> dict:
    ...
    print()
    print(
        "MEASURES: This assessment evaluates the patient's ability to think abstractly, interpret"
    )
    ...
    responses["stitch_meaning"] = input(
        "What does 'A stitch in time saves nine' mean to you?: "
    ).strip()
    responses["stitch_example"] = input(
        "Can you give an example of how this applies to life?: "
    ).strip()
    responses["bird_meaning"] = input(
        "What does 'A bird in the hand is worth two in the bush' mean?: "
    ).strip()
    responses["rolling_meaning"] = input(
        "What does 'A rolling stone gathers no moss' mean?: "
    ).strip()

    # FABLES
    print("\n--- FABLE AND STORY COMPREHENSION ---")
    responses["story_lesson"] = input(
        "If you heard a story about a tortoise and hare racing, what lesson would it teach?: "
    ).strip()
    responses["character_action"] = input(
        "Why do you think characters in stories behave the way they do?: "
    ).strip()

    # METAPHORS AND ANALOGIES
    print("\n--- METAPHORS AND ANALOGIES ---")
    responses["metaphor"] = input(
        "What does it mean when someone says 'Life is a journey'?: "
    ).strip()
    ...
    # CONCEPTUAL THINKING
    print("\n--- ABSTRACT THINKING ---")
    responses["abstract_concept"] = input(
        "What do happiness and sadness have in common?: "
    ).strip()
    responses["pattern"] = input(
        "What pattern do you see in: 2, 4, 8, 16, ___?: "
    ).strip()
    responses["hypothesis"] = input(
        "If all birds could not fly, how would the world be different?: "
    ).strip()

    # COGNITIVE STYLE
    print("\n--- COGNITIVE STYLE ---")
    responses["abstract_comfort"] = input(
        "Do you prefer to think in concrete facts or abstract ideas? (concrete/abstract/both): "
    ).strip()
    responses["perspective_shift"] = input(
        "Can you easily see things from different perspectives? (easily/sometimes/difficult): "
    ).strip()

    return responses


def create_abstract_reasoning_assessment_from_responses(
    ...
    if output_path is None:
        output_path = (
            Path("outputs")
            / f"{patient_name.lower().replace(' ', '_')}_abstract_reasoning.json"
        )
...
def evaluate_abstract_reasoning(
    ...
    assessment = create_abstract_reasoning_assessment_from_responses(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default="unknown",
        help="Name or identifier of the patient",
    )
    parser.add_argument(
        "-o",
        "--output",
