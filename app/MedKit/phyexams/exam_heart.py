...
class NurseReport(BaseModel):
    vitals: dict = Field(
        ...,
        description="Heart rate, blood pressure, respiratory rate, oxygen saturation",
    )
    inspection: dict = Field(
        ..., description="Observation: jugular venous distention, cyanosis, edema"
    )
    palpation: dict = Field(..., description="Palpation: apex beat, thrills, heaves")
    auscultation: dict = Field(
        ..., description="Heart sounds: S1, S2, murmurs, extra sounds"
    )
    peripheral_signs: dict = Field(
        ..., description="Peripheral pulses, capillary refill, edema"
    )
...
class HeartExam(BaseModel):
    ...
    patient_answers: Optional[List[PatientAnswer]] = Field(
        None, description="Patient answers to heart-related questions"
    )
