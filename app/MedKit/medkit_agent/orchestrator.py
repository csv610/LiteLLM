...
class MedKitOrchestrator:
    ...
    def __init__(self, model: str = "ollama/gemma3", temperature: float = 0.2, max_steps: int = 5):
        self.model = model
...
"""
        ...
        return [
            {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        "medicine_name": {"type": "string", "description": "Generic name of the drug"}
                    },
            ...
            },
            {
                ...
                "parameters": {
                    ...
                    "properties": {
                        "entity_type": {
                            "type": "string", 
                            "enum": ["drug", "disease", "pathogen", "clinical_sign", "medical_symptom", "medical_test"]
                        },
            ...
            },
            {
                ...
                "parameters": {
                    "type": "object",
                    "properties": {
                        "query": {"type": "string", "description": "The medical condition to search for"}
                    },
                    "required": ["query"]
                }
            }
        ...
        try:
            if tool_name == "get_medicine_info":
                ...
                if not client_id or not client_secret:
                    return ToolOutput(tool_name=tool_name, status="error", result="ICD-11 API credentials missing in environment.")
                ...
                if results and 'destinationEntities' in results:
                    ...
                    for entity in results['destinationEntities'][:5]:
                        title = entity.get('title', '').replace('<em class=\'found\'>', '').replace('</em>', '')
                        summary.append(f"Code: {entity.get('theCode', 'N/A')} | {title}")
                    return ToolOutput(tool_name=tool_name, result="\n".join(summary))
                return ToolOutput(tool_name=tool_name, result="No matching ICD-11 codes found.")
            
            return ToolOutput(tool_name=tool_name, result=f"Tool '{tool_name}' not implemented.", status="error")
    ...
    def run(self, query: str):
        ...
        for step in range(self.max_steps):
            ...
            if function_call:
                tool_name = function_call["name"] if isinstance(function_call, dict) else function_call.name
