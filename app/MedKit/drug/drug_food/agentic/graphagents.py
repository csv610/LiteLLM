"""
graphagents.py - Multi-agent implementation using LangGraph for drug_food.
"""
...
class GraphDrugFoodAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for drug_food: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for drug_food synthesize: {state['specialist_output']}")
