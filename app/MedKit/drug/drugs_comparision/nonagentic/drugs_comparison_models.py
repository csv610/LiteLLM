...
class PracticalMetrics(BaseModel):
    ...
    availability_status: AvailabilityStatus = Field(
        description="Prescription, OTC, controlled, etc."
    )
    typical_cost_range: str = Field(description="Typical cost range without insurance")
    insurance_coverage: str = Field(
        description="How typically covered by major insurance"
    )
    available_formulations: str = Field(
        description="Available pharmaceutical forms, comma-separated"
    )
    dosage_strengths: str = Field(
        description="Available dosage strengths, comma-separated"
    )
    generic_cost: Optional[str] = Field(
        default=None, description="Typical cost of generic version if available"
    )
