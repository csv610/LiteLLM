"""
graphagents.py - Multi-agent implementation using LangGraph for drugs_comparision.
"""
...
class GraphDrugsComparisionAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for drugs_comparision: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for drugs_comparision synthesize: {state['specialist_output']}")
