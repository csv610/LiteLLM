...
class ChemicalProperties(BaseModel):
    ...
    monoisotopic_weight: Optional[float] = Field(
        None, description="Monoisotopic weight"
    )
...
class Taxonomy(BaseModel):
    ...
    alternative_parents: Optional[List[str]] = Field(
        default_factory=list, description="Alternative parents"
    )
...
class ExternalIdentifier(BaseModel):
    ...
    database: str = Field(
        ..., description="Database name (e.g., PubChem, ChEMBL, KEGG)"
    )
...
class Patent(BaseModel):
    ...
    pediatric_extension: Optional[bool] = Field(
        None, description="Has pediatric extension"
    )
...
class Interaction(BaseModel):
    ...
    drugbank_id: Optional[str] = Field(
        None, description="DrugBank ID of interacting drug"
    )
    description: str = Field(..., description="Interaction description")
    severity: Optional[str] = Field(
        None, description="Severity level (major, moderate, minor)"
    )
...
class Target(BaseModel):
    ...
    action: Optional[str] = Field(
        None, description="Type of action (inhibitor, agonist, etc.)"
    )
    gene_name: Optional[str] = Field(None, description="Gene name")
    uniprot_id: Optional[str] = Field(None, description="UniProt ID")
    pharmacological_action: Optional[bool] = Field(
        None, description="Is pharmacologically active"
    )
...
class Pharmacodynamics(BaseModel):
    ...
    mechanism_of_action: Optional[str] = Field(
        None, description="Detailed mechanism of action"
    )
    pharmacodynamics: Optional[str] = Field(
        None, description="Pharmacodynamic description"
    )
...
class Dosage(BaseModel):
    ...
    dosage_instructions: Optional[str] = Field(
        None, description="Detailed dosing instructions"
    )
...
class BasicInfo(BaseModel):
    ...
    synonyms: List[str] = Field(
        default_factory=list, description="Alternative names and brand names"
    )
    description: Optional[str] = Field(
        None, description="Comprehensive drug description"
    )
    chemical_properties: Optional[ChemicalProperties] = Field(
        None, description="Chemical properties"
    )
    affected_organisms: Optional[List[str]] = Field(
        default_factory=list, description="Affected organisms"
    )
    synthesis_reference: Optional[str] = Field(
        None, description="Chemical synthesis information"
    )


class Classification(BaseModel):
    drug_type: Optional[DrugType] = Field(None, description="Type of drug")
    groups: List[DrugGroup] = Field(
        default_factory=list, description="Drug approval groups"
    )
    categories: List[str] = Field(
        default_factory=list, description="Therapeutic categories"
    )
    atc_codes: List[ATCCode] = Field(
        default_factory=list, description="ATC classification codes"
    )
    taxonomy: Optional[Taxonomy] = Field(None, description="Chemical taxonomy")


class Pharmacology(BaseModel):
    pharmacodynamics: Optional[Pharmacodynamics] = Field(
        None, description="Pharmacodynamic properties"
    )
    pharmacokinetics: Optional[Pharmacokinetics] = Field(
        None, description="Pharmacokinetic properties"
    )


class Indications(BaseModel):
    indication: Optional[str] = Field(None, description="Approved indications")
    off_label_uses: Optional[List[str]] = Field(
        default_factory=list, description="Off-label uses"
    )


class Administration(BaseModel):
    dosages: List[Dosage] = Field(
        default_factory=list, description="Available dosage forms"
    )
    administration: Optional[str] = Field(
        None, description="Administration instructions"
    )
...
class Interactions(BaseModel):
    targets: List[Target] = Field(
        default_factory=list, description="Biological targets"
    )
    enzymes: List[Enzyme] = Field(
        default_factory=list, description="Metabolizing enzymes"
    )
    carriers: List[Carrier] = Field(
        default_factory=list, description="Carrier proteins"
    )
    transporters: List[Transporter] = Field(
        default_factory=list, description="Transporter proteins"
    )
    drug_interactions: List[Interaction] = Field(
        default_factory=list, description="Drug-drug interactions"
    )
    food_interactions: List[FoodInteraction] = Field(
        default_factory=list, description="Drug-food interactions"
    )
...
class Safety(BaseModel):
    ...
    adverse_reactions: List[AdverseReaction] = Field(
        default_factory=list, description="Adverse reactions"
    )
...
class Regulation(BaseModel):
    ...
    patents: List[Patent] = Field(
        default_factory=list, description="Patent information"
    )
    manufacturers: List[Manufacturer] = Field(
        default_factory=list, description="Manufacturers"
    )
    pricing: List[PricingInfo] = Field(
        default_factory=list, description="Pricing information by country"
    )


class References(BaseModel):
    external_identifiers: List[ExternalIdentifier] = Field(
        default_factory=list, description="External database IDs"
    )
    clinical_trials: List[ClinicalTrial] = Field(
        default_factory=list, description="Associated clinical trials"
    )
    literature_references: Optional[List[str]] = Field(
        default_factory=list, description="PubMed IDs and references"
    )
...
class MedicineInfo(BaseModel):
    ...
    classification: Optional[Classification] = Field(
        None, description="Drug classification"
    )
    pharmacology: Optional[Pharmacology] = Field(
        None, description="Pharmacology information"
    )
    indications: Optional[Indications] = Field(None, description="Indications for use")
    administration: Optional[Administration] = Field(
        None, description="Administration guidelines"
    )
    interactions: Optional[Interactions] = Field(
        None, description="Drug interactions and targets"
    )
