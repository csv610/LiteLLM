...
class WithdrawalSymptomModel(BaseModel):
    ...
    severity: WithdrawalSeverity = Field(
        description="Severity of this specific symptom"
    )
    duration: Optional[str] = Field(
        default=None, description="Typical duration of the symptom"
    )
    management: Optional[str] = Field(
        default=None, description="How to manage this symptom"
    )
...
class AddictionMechanismModel(BaseModel):
    ...
    neurotransmitter_impact: str = Field(
        description="Affected neurotransmitters (e.g., Dopamine, GABA)"
    )
...
class DrugAddictionDetailsModel(BaseModel):
    ...
    withdrawal_symptoms: List[WithdrawalSymptomModel] = Field(
        description="List of withdrawal symptoms"
    )
    long_term_effects: List[str] = Field(
        description="Long-term physical and mental effects"
    )
    risk_factors: List[str] = Field(
        description="Factors that increase the risk of addiction"
    )
    treatment_options: List[str] = Field(
        description="Commonly used treatment and recovery options"
    )
    prevention_strategies: List[str] = Field(
        description="Strategies to prevent addiction or dependence"
    )
    confidence_level: ConfidenceLevel = Field(
        description="Confidence level in this assessment"
    )
    references: Optional[str] = Field(
        default=None, description="Citations or references supporting this data"
    )
...
class PatientFriendlyAddictionSummaryModel(BaseModel):
    ...
    signs_of_addiction: List[str] = Field(
        description="Warning signs for the patient or family"
    )
    what_to_do: str = Field(description="Immediate steps if addiction is suspected")
    recovery_outlook: str = Field(
        description="General outlook for recovery and support"
    )
...
class DrugAddictionModel(BaseModel):
    ...
    addiction_details: Optional[DrugAddictionDetailsModel] = Field(
        default=None, description="Detailed addiction information"
    )
