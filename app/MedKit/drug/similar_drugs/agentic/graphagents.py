"""
graphagents.py - Multi-agent implementation using LangGraph for similar_drugs.
"""
...
class GraphSimilarDrugsAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for similar_drugs: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for similar_drugs synthesize: {state['specialist_output']}")
