...
class SimilarMedicineDetail(BaseModel):
    ...
    brand_names: Optional[str] = Field(
        default=None, description="Brand names or trade names, comma-separated"
    )
...
class SimilarMedicinesCategory(BaseModel):
    ...
    count: int = Field(
        description="Number of similar medicines in this category (from top 10-15)"
    )
...
class SimilarMedicinesResult(BaseModel):
    ...
    original_medicine: str = Field(
        description="Name of the original medicine being analyzed"
    )
...
class TriageResultModel(BaseModel):
    """Result of the initial triage by the Triage Agent."""

    is_real_medicine: bool = Field(
        description="Whether the input appears to be a real medicine name"
    )
    has_alternatives: bool = Field(
        description="Whether there are likely to be similar alternatives available"
    )
    initial_reasoning: str = Field(
        description="Brief reasoning for the triage decision"
    )
    suggested_focus_areas: list[str] = Field(
        description="Key areas for the research agents to focus on"
    )


class ComplianceInfoModel(BaseModel):
    """Compliance and safety review information."""

    compliance_passed: bool = Field(
        description="Whether the generated information meets medical safety standards"
    )
    safety_warnings: list[str] = Field(
        description="Critical safety warnings that must be included"
    )
    disclaimers: list[str] = Field(description="Required medical disclaimers")
    compliance_notes: str = Field(description="Notes from the compliance review")


class AuditLogModel(BaseModel):
    """Detailed logs from each agent's contribution."""

    triage_raw: Optional[TriageResultModel] = None
    research_raw: Optional[SimilarMedicinesResult] = None
    compliance_raw: Optional[ComplianceInfoModel] = None


class SimilarMedicinesModel(BaseModel):
    """Final consolidated result from the multi-agent orchestration."""

    main_result: SimilarMedicinesResult
