"""
graphagents.py - Multi-agent implementation using LangGraph for symptoms_drugs.
"""
...
class GraphSymptomsDrugsAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for symptoms_drugs: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for symptoms_drugs synthesize: {state['specialist_output']}")
