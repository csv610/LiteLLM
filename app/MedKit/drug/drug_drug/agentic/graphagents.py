"""
graphagents.py - Multi-agent implementation using LangGraph for drug_drug.
"""
...
class GraphDrugDrugAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for drug_drug: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for drug_drug synthesize: {state['specialist_output']}")
