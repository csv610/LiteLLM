"""
agnoagents.py - Multi-agent implementation using Agno framework for drug_drug.
"""
...
from app.MedKit.drug.drug_drug.shared.models import ModelOutput
...
class AgnoDrugDrugAgents:
    def __init__(self, model_name: str = "gemini/gemini-2.5-flash"):
        self.model_name = model_name
        self.model = get_agno_model(model_name)
        
        self.specialist = Agent(
            name="drug_drug Specialist",
            role="Expert in drug_drug tasks",
            model=self.model,
            instructions=["Provide deep analysis and structured insights for drug_drug."],
        )

        self.verifier = Agent(
            name="drug_drug Verifier",
        ...
        self.manager = Agent(
            name="drug_drug Manager",
            team=[self.specialist, self.verifier],
            model=self.model,
            instructions=[
                "Coordinate the drug_drug workflow.",
