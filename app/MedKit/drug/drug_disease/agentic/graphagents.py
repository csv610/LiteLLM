"""
graphagents.py - Multi-agent implementation using LangGraph for drug_disease.
"""
...
class GraphDrugDiseaseAgents:
    ...
    def specialist_node(self, state: AgentState):
        response = self.llm.invoke(f"Specialist for drug_disease: {state['input']}")
        return {"specialist_output": response.content}

    def manager_node(self, state: AgentState):
        response = self.llm.invoke(f"Manager for drug_disease synthesize: {state['specialist_output']}")
