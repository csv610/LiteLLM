...
class GraphQuadrailsAgents:
    """LangGraph-based multi-agent implementation for Quadrails."""
    ...
    def specialist_node(self, state: AgentState):
        prompt = f"You are a Specialist for Quadrails. Analyze this input: {state['input']}"
        response = self.llm.invoke(prompt)
        return {"specialist_output": response.content}

    def verifier_node(self, state: AgentState):
        prompt = f"You are a Verifier for Quadrails. Interrogate this analysis: {state['specialist_output']}"
        response = self.llm.invoke(prompt)
        return {"verifier_output": response.content}

    def manager_node(self, state: AgentState):
        prompt = f"""Synthesize the following into a final Markdown report for Quadrails:
        Input: {state['input']}
