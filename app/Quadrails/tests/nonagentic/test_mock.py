...
def mock_generate_text():
    with patch("lite.lite_client.LiteClient.generate_text") as mock:
        yield mock
...
def test_analyze_text_safe(mock_generate_text, safe_response):
    mock_generate_text.return_value = safe_response
...
def test_analyze_text_caching(mock_generate_text, safe_response):
    mock_generate_text.return_value = safe_response
...
def test_analyze_text_preprocessing(mock_generate_text, safe_response):
    mock_generate_text.return_value = safe_response
...
def test_analyze_text_flagged(mock_generate_text, flagged_response):
    mock_generate_text.return_value = flagged_response
...
def test_analyze_image_unsafe(mock_generate_text, nudity_violence_image_response):
    mock_generate_text.return_value = nudity_violence_image_response

    analyzer = GuardrailAnalyzer()

    with patch("Quadrails.nonagentic.guardrail.Path.exists", return_value=True):
        with patch("Quadrails.nonagentic.guardrail.Path.absolute", return_value=Path("/path/to/unsafe.jpg")):
