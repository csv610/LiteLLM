...
@patch('guardrail_cli.GuardrailAnalyzer')
def test_main_with_text(mock_analyzer_class, capsys):
    mock_analyzer = mock_analyzer_class.return_value
    mock_result = MagicMock()
    mock_analyzer.analyze_text = AsyncMock(return_value=mock_result)
    
    with patch.object(sys, 'argv', ['guardrail_cli.py', '-t', 'some text']):
        main()
        
    mock_analyzer.analyze_text.assert_called_with('some text', use_cache=True)
...
def test_main_with_no_cache(mock_analyzer_class, capsys):
    ...
    with patch.object(sys, 'argv', ['guardrail_cli.py', '-t', 'some text', '--no-cache']):
        main()
