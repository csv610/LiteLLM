...
async def run_analysis(args: argparse.Namespace):
    ...
    try:
        ...
        logging_config.configure_logging(str(Path(__file__).parent.parent / "logs" / "guardrail_cli.log"))
        
        # Initialize analyzer with specified model
        model_config = ModelConfig(model=args.model, temperature=0.1)
        analyzer = GuardrailAnalyzer(model_config, max_length=args.max_length)
        ...
        if args.text:
            ...
            result = await analyzer.analyze_image(args.image, use_cache=not args.no_cache)
        else:
