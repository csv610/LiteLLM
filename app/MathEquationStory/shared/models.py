...
class ResearchBrief(BaseModel):
    ...
    historical_context: str = Field(
        ..., description="Who discovered it, when, and the problem they were solving"
    )
    mathematical_core: str = Field(
        ...,
        description="The fundamental mathematical principles and logic behind the equation",
    )
    real_world_applications: List[str] = Field(
        ..., description="3-5 concrete ways this equation is used today"
    )
    key_metaphors: List[str] = Field(
        ..., description="Potential analogies or metaphors to help explain the concept"
    )
    common_misconceptions: List[str] = Field(
        ..., description="Common ways people misunderstand this equation"
    )
...
class MathematicalEquationStory(BaseModel):
    ...
    story: str = Field(
        ...,
        description="The complete narrative story as flowing prose, written like a professional science article",
    )
    vocabulary_notes: str = Field(
        ..., description="Brief explanations of technical terms used in the story"
    )
