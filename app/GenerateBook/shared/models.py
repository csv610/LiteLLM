...
class PlannedLevel(BaseModel):
    ...
    sequencing_rationale: str = Field(..., description="Why this order fits the audience.")
    chapter_outline: list[str] = Field(..., description="Ordered chapter titles or themes.")
...
class CurriculumPlanModel(BaseModel):
    ...
    levels: list[PlannedLevel] = Field(..., description="Planned structure for each requested level.")
...
class BookChaptersModel(BaseModel):
    ...
    agent_trace: Optional[AgentTrace] = Field(default=None, description="Metadata from the three-agent workflow.")


class ReviewIssue(BaseModel):
    """One issue found by the reviewer."""
    level: str = Field(..., description="The affected education level.")
    chapter_number: Optional[int] = Field(default=None, description="Affected chapter if applicable.")
