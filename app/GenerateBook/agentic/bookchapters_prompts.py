"""PromptBuilder for the three-agent curriculum generation workflow."""
...
class PromptBuilder:
    """Builder class for three-agent curriculum prompts."""
    ...
    @staticmethod
    def _requested_levels(level: str = None) -> list[str]:
        """Return the requested level list for prompt construction."""
        return [level] if level else EDUCATION_LEVELS
...
- Reading level: General audience accessible material"""
    ...
    @staticmethod
    def get_planner_prompt(subject: str, level: str = None, num_chapters: int = 12) -> str:
        """Prompt for the planner agent."""
        requested_levels = ", ".join(PromptBuilder._requested_levels(level))
        return f"""You are the planner agent in a 3-agent curriculum system.

Your job is to design the structure before anyone writes full chapter details.

Subject: {subject}
Requested levels: {requested_levels}
Chapters per level: {num_chapters}

Return a curriculum plan that:
- Covers only the requested levels
- Provides one ordered chapter outline per level with exactly {num_chapters} items
- Explains the audience, age range, and sequencing rationale for each level
- Keeps progression developmentally appropriate and internally consistent

Do not generate full chapter summaries, experiments, or projects yet. Produce only the planning structure."""

    @staticmethod
    def get_generator_prompt(subject: str, num_chapters: int, plan_json: str) -> str:
        """Prompt for the generator agent using the planner output."""
        return f"""You are the generator agent in a 3-agent curriculum system.

Your job is to turn the approved plan into a complete curriculum.
...
Generate the full curriculum and follow the planner's structure closely.
You may improve wording, but do not change the requested number of chapters or the intended audience fit.
For every level in the plan, generate detailed chapters with measurable learning objectives, observations, experiments, and projects."""

    @staticmethod
    def get_reviewer_prompt(subject: str, plan_json: str, draft_json: str) -> str:
        """Prompt for the reviewer agent using both plan and draft."""
        return f"""You are the reviewer agent in a 3-agent curriculum system.

Your task is to review the generated curriculum against the original plan and return a corrected final version.
...
- chapter count and sequencing consistency
- alignment with the planner's intent
- clear, measurable learning objectives
- realistic prerequisites, observations, experiments, and projects
- internal consistency across the full curriculum

Fix problems directly in the final curriculum you return. Keep useful content, but correct weak or inconsistent material."""
