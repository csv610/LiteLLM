...
class RiemannOverviewModel(BaseModel):
    ...
    layperson_explanation: str = Field(
        description="A simple, non-mathematical explanation for a layperson"
    )
...
class RiemannTechnicalModel(BaseModel):
    ...
    misconceptions: List[str] = Field(
        description="Common myths and the reality behind them"
    )
    limitations: str = Field(description="Boundaries and unsolved obstacles")
    modern_developments: str = Field(
        description="How the theory has progressed since Riemann"
    )
...
class RiemannAppliedImpactModel(BaseModel):
    ...
    applications: List[str] = Field(
        description="Practical or theoretical applications in science/tech"
    )
    significance: str = Field(description="Broad significance in modern science")
    counterfactual_impact: str = Field(
        description="How the world would be different if this wasn't developed"
    )


class RiemannTheoryModel(
    RiemannOverviewModel, RiemannTechnicalModel, RiemannAppliedImpactModel
):
...
logging_config.configure_logging(
    str(Path(__file__).parent / "logs" / "riemann_theory.log")
)
...
class RiemannTheoryGuide:
    ...
    def save_to_file(
        self, output: ModelOutput, theory_name: str, output_dir: str
    ) -> str:
        """
    ...
    def display_summary(self):
        ...
        try:
            ...
            model_input = ModelInput(
                system_prompt=PromptBuilder.get_system_prompt(), user_prompt=user_prompt
            )
...
class MultiAgentRiemannGuide:
    ...
    async def overview_agent(
        self, theory_name: str, brief: str
    ) -> RiemannOverviewModel:
    ...
    async def technical_agent(
        ...
        print(
            f"🎓 Technical Agent (Professor): Drafting formal math for {theory_name}..."
        )
        model_input = ModelInput(
    ...
    async def app_expert_agent(
        self, theory_name: str, tech_report: RiemannTechnicalModel
    ) -> RiemannAppliedImpactModel:
    ...
    async def critic_agent(
        ...
        print(
            f"🧐 Critic Agent: Synthesizing final Markdown dossier for {theory_name}..."
        )
        dossier_data = {
            "theory": theory_name,
    ...
    async def run_pipeline(self, theory_name: str) -> str:
        ...
        final_markdown = await self.critic_agent(
            theory_name, overview, technical, applied
        )
