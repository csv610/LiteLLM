...
__all__ = ["HilbertProblemResponse", "HilbertProblemsGuide"]
...
def _configure_module_logging() -> None:
    ...
    logging_config.configure_logging(
        str(Path(__file__).parent / "logs" / "hilbert_problems.log")
    )
...
class HilbertProblemsGuide:
    ...
    def _validate_problem_number(problem_number: int) -> None:
        if not 1 <= problem_number <= 23:
            raise ValueError(
                f"Problem number must be between 1 and 23, got {problem_number}"
            )

    def _run_structured_agent(
        self, client: LiteClient, role_name: str, system_prompt: str, user_prompt: str
    ...
    def _generate_problem_draft(
        self, problem_number: int
    ) -> Optional[HilbertProblemModel]:
    ...
    def _review_problem_draft(
        self, problem_number: int, draft_problem: HilbertProblemModel
    ) -> Optional[str]:
    ...
    def generate_text(self, problem_number: int) -> ModelOutput:
        ...
        try:
            ...
            if draft_problem is None:
                logger.error(
                    f"Researcher agent did not return structured output for problem {problem_number}"
                )
                return ModelOutput(metadata={"error": "Researcher failed"})

            reviewed_problem_md = self._review_problem_draft(
                problem_number, draft_problem
            )
            if reviewed_problem_md is None:
                ...
                return ModelOutput(
                    data=draft_problem, metadata={"error": "Reviewer failed"}
                )
    ...
    def _generate_summary_draft(self) -> Optional[str]:
        titles_context = "\n".join(
            [f"#{n}: {t}" for n, t in PromptBuilder.PROBLEM_TITLES.items()]
        )
    ...
    def save_to_file(
        self, problem_md: str, problem_number: int, output_dir: str
    ) -> str:
        try:
            output_path = Path(output_dir)
            output_path.mkdir(parents=True, exist_ok=True)

            title = PromptBuilder.PROBLEM_TITLES.get(
                problem_number, f"Problem_{problem_number}"
            )
    ...
    def display_summary(self):
        ...
        try:
            ...
            if reviewed_summary is None:
                logger.warning(
                    "Reviewer agent did not return summary text; falling back to researcher draft"
                )
