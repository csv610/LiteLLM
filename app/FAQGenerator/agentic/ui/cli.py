...
def arguments_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Generate FAQs with a two-agent workflow: generator plus reviewer",
...
def main() -> int:
    ...
    try:
        ...
        configure_logging(log_file=str(Path(__file__).parent.parent / "logs" / "faq_generator.log"))
        ...
        print(
            f"Running 2-agent FAQ workflow for {args.num_faqs} {args.difficulty} FAQs "
            f"from {source_type} '{args.input_source}'..."
        )

        # Generate FAQs
        output_artifact = generator.generate_text(faq_input)

        if not output_artifact or not output_artifact.markdown:
            logger.error("No FAQ content returned from API")
            return 1

        # Save to file using DataExporter
        output_file = DataExporter.export_to_markdown(output_artifact.markdown, faq_input)

        print(f"FAQ generation complete. Saved to {output_file}")
        
        # Also print to console
        print("\n--- GENERATED FAQs (MARKDOWN) ---\n")
        print(output_artifact.markdown)
        return 0
