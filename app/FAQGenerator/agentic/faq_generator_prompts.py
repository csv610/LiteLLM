...
Contains the PromptBuilder class for creating prompts for a two-agent FAQ
workflow: a generator agent drafts FAQs and a reviewer agent validates and
repairs them.
"""
...
class PromptBuilder:
    """Build prompts for generator and reviewer agents."""
...
2. Answer: A comprehensive, academically rigorous answer grounded in peer-reviewed literature, established theories, and empirical evidence in the field
    ...
    def build_review_prompt(self, source: str, faqs: list[FAQ], content: str | None = None) -> str:
        """
        Build prompt for reviewer agent to validate and repair generated FAQs.

        Args:
            source: Original topic or file path
            faqs: Draft FAQ objects from generator agent
            content: Optional source content if generation was file-based

        Returns:
            Formatted review prompt string
        """
        faq_payload = json.dumps([faq.model_dump() for faq in faqs], indent=2)
        source_context = "file-derived content" if content else f"topic '{source}'"
        content_clause = ""
        if content:
            content_clause = f"\nReference content the generator analyzed:\n{content}\n"

        return f"""You are the reviewer agent in a two-agent FAQ generation system.

You are reviewing a draft FAQ set produced for {source_context}.{content_clause}
Draft FAQs:
{faq_payload}

Review requirements:
- Preserve the same difficulty level and target FAQ count unless a draft is invalid and needs replacement
- Remove semantic duplicates and near-duplicates
- Fix weak, vague, subjective, or non-interrogative questions
- Fix answers that are too short, unclear, or not academically objective
- Ensure each FAQ is self-contained and materially different from the others
- If any FAQ is unsalvageable, replace it with a better one that fits the same topic and difficulty
- Return the final polished FAQ set only

{self.SEMANTIC_DIVERSITY}

{self.QUALITY_CRITERIA}

Also include a short reviewer_notes field summarizing what you validated or changed."""
