...
def temp_outputs(tmp_path):
    ...
    with patch("StudyGuide.nonagentic.studyguide_generator.Path") as mock_path:
        # We need to make sure Path(__file__).parent / "outputs" works
        mock_path.return_value.parent.__truediv__.return_value = tmp_path
        yield tmp_path


@patch("app.StudyGuide.nonagentic.studyguide_generator.LiteClient")
def test_studyguide_generator_init(MockLiteClientClass):
    from .studyguide_generator import StudyGuideGenerator
    config = ModelConfig(model="test-model")
    generator = StudyGuideGenerator(config)
    assert generator.model == "test-model"
    assert MockLiteClientClass.called


@patch("app.StudyGuide.nonagentic.studyguide_generator.LiteClient")
def test_generate_and_save_mock(MockLiteClientClass, tmp_path):
    from .studyguide_generator import StudyGuideGenerator
    # Setup mock responses
    mock_instance = MockLiteClientClass.return_value

    def mock_generate_text(*args, **kwargs):
        model_input = kwargs.get('model_input') or (args[0] if args else None)
        if not model_input:
            return None
        ...
        if fmt == SummaryPlanModel:
            ...
            return ResearchModel(
                latest_updates=[
                    ResearchUpdate(
                        title="Update 1", summary="Sum 1", source_citation="Cit 1"
                    )
                ],
            ...
            return MindMapModel(
                mermaid_code="graph TD; A-->B", map_description="Map Desc"
            )
        elif fmt == BatchSummaryResponse:
            return BatchSummaryResponse(
                chapters=[
                    ChapterSummaryAndAnalysis(
                        chapter_title="Chapter 1", summary="Sum 1", analysis="Ana 1"
                    ),
                    ChapterSummaryAndAnalysis(
                        chapter_title="Chapter 2", summary="Sum 2", analysis="Ana 2"
                    ),
                ]
            )
            ...
            return RelevancyModel(
                modern_perspectives=[
                    ModernPerspective(point="Point 1", explanation="Exp 1")
                ],
            ...
            return FollowUpModel(
                further_reading=[
                    ReadingRecommendation(
                        title="Rec 1", author="Aut 1", why_it_relates="Why 1"
                    )
                ],
                actionable_next_steps=["Step 1"],
                media_connections=[
                    MediaItem(type="Film", title="Film 1", description="Desc 1")
                ],
            )
        return None
    ...
    with patch("app.StudyGuide.nonagentic.studyguide_generator.Path") as mock_path_class:
