...
from app.StudyGuide.shared.models import ModelOutput
...
class GraphStudyGuideAgents:
    """LangGraph-based multi-agent implementation for StudyGuide."""
    ...
    def specialist_node(self, state: AgentState):
        prompt = f"You are a Specialist for StudyGuide. Analyze this input: {state['input']}"
        response = self.llm.invoke(prompt)
        return {"specialist_output": response.content}

    def verifier_node(self, state: AgentState):
        prompt = f"You are a Verifier for StudyGuide. Interrogate this analysis: {state['specialist_output']}"
        response = self.llm.invoke(prompt)
        return {"verifier_output": response.content}

    def manager_node(self, state: AgentState):
        prompt = f"""Synthesize the following into a final Markdown report for StudyGuide:
        Input: {state['input']}
