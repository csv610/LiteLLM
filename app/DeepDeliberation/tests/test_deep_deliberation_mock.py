...
from DeepDeliberation.noagentic.deep_deliberation_models import (
    InitialKnowledgeMap,
    DiscoveryFAQ,
    DiscoveryInsight,
    DiscoveryCheck,
...
class TestDeepDeliberation(unittest.TestCase):
    def setUp(self):
        ...
        with (
            patch("DeepDeliberation.noagentic.deep_deliberation.LiteClient"),
            patch("DeepDeliberation.noagentic.deep_deliberation.DiscoveryAgent"),
        ):
    ...
    def test_execute_single_probe_success(self):
        ...
        verification = VerificationResult(
            is_verified=True, credibility_score=9, critique="Solid"
        )
        summary = "Summary"
    ...
    def test_run_integration(self):
        ...
        self.orchestrator.agent.analyze.return_value = insight
        self.orchestrator.agent.check_novelty.return_value = DiscoveryCheck(
            is_novel=True, discovery_score=9, reasoning="Good"
        )
        self.orchestrator.agent.verify.return_value = VerificationResult(
            is_verified=True, credibility_score=9, critique="Solid"
        )
        self.orchestrator.agent.summarize.return_value = "S1"

        with patch(
            "DeepDeliberation.noagentic.deep_deliberation.MissionArchive"
        ) as mock_archive:
