...
def main() -> int:
    ...
    try:
        ...
        output_artifact = run_discovery_mission(
            args.topic, 
            args.num_rounds, 
            args.num_faqs, 
            model=args.model, 
        ...
        print("\n--- STRATEGIC KNOWLEDGE MAP (Markdown Report) ---\n")
        if hasattr(output_artifact, 'markdown'):
            print(output_artifact.markdown)
        else:
            print(output_artifact)
            
        print(f"\nMission complete. Data archived to: {output_filename} (and .md)")
