...
class MillenniumProblem(BaseModel):
    ...
    field: str = Field(
        ..., description="The mathematical or physics field problem belongs to"
    )
    status: str = Field(..., description="Current status (Unsolved or Solved)")
    solver: Optional[str] = Field(
        None, description="Name of solver if problem has been solved"
    )
    year_solved: Optional[int] = Field(None, description="Year problem was solved")
    significance: str = Field(
        ..., description="Why this problem is important and its implications"
    )
    current_progress: str = Field(
        ..., description="Recent progress, approaches, or partial results"
    )
...
class MillenniumProblemsResponse(BaseModel):
    ...
    total_problems: int = Field(
        ..., description="Total number of Millennium Prize Problems"
    )
    problems: List[MillenniumProblem]


MillenniumProblemExplanation = MillenniumProblem
