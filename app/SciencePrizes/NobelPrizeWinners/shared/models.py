...
class PersonalBackground(BaseModel):
    ...
    birth_date: str = Field(
        ...,
        description="Birth date (format: YYYY-MM-DD or approximate year if exact date unknown)",
    )
    ...
    family_background: str = Field(
        ...,
        description="Family background, parents' professions, and early influences. Include relevant family members who influenced their career.",
    )
    education: list[str] = Field(
        ...,
        description="Chronological list of educational institutions and degrees (e.g., 'B.Sc. Physics, University of X (1975)', 'Ph.D. Chemistry, University of Y (1980)')",
        min_length=1,
    )
    early_influences: str = Field(
        ...,
        description="Key people, mentors, or experiences that shaped their scientific interests and career direction during formative years",
    )
...
class CareerPosition(BaseModel):
    ...
    title: str = Field(
        ...,
        description="Job title or position (e.g., 'Professor of Physics', 'Research Scientist')",
    )
    ...
    start_year: int = Field(
        ..., description="Start year of position", ge=1900, le=CURRENT_YEAR
    )
    end_year: int | None = Field(
        None, description="End year of position (null if current)"
    )
...
class BroaderRecognition(BaseModel):
    ...
    honors_and_awards: list[str] = Field(
        ...,
        description="List of major awards and honors received (excluding Nobel Prize)",
        min_length=1,
    )
    academy_memberships: list[str] = Field(
        ...,
        description="Memberships in prestigious academies and societies (e.g., 'Fellow of Royal Society', 'Member of National Academy of Sciences')",
        min_length=1,
    )
    editorial_roles: list[str] = Field(
        ...,
        description="Editorial positions in scientific journals and publications",
        min_length=0,
    )
    mentorship_contributions: str = Field(
        ...,
        description="Notable students, postdocs, and collaborators mentored; their achievements and contributions to science",
    )
    leadership_roles: list[str] = Field(
        ...,
        description="Leadership positions in scientific organizations, departments, or research initiatives",
        min_length=0,
    )
    public_engagement: str = Field(
        ...,
        description="Public communication, outreach efforts, scientific advocacy, or policy influence. Include speaking engagements, public lectures, and involvement in science communication.",
    )
...
class PrizeWinner(BaseModel):
    ...
    year: int = Field(
        ..., description="Year of prize was awarded", ge=1901, le=CURRENT_YEAR
    )
    category: str = Field(
        ..., description="Nobel Prize category (Physics, Chemistry, Medicine, etc.)"
    )
    ...
    personal_background: PersonalBackground = Field(
        ..., description="Early life, education, and family background of the laureate"
    )
    career_timeline: list[CareerPosition] = Field(
        ...,
        description="Chronological list of major career positions and institutional affiliations",
        min_length=1,
    )
    broader_recognition: BroaderRecognition = Field(
        ...,
        description="Honors, recognition, and broader roles in the scientific community outside of the Nobel Prize",
    )
