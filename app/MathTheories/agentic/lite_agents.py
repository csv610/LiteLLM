...
from app.MathTheories.shared.utils import save_result, print_result
...
class MathTheoryExplainer:
    ...
    def generate_text(
        ...
        for level in audience_levels:
            self.logger.info(
                f"Generating Tier 1/2 explanation for audience: {level.value}"
            )
            ...
            while attempt < self.max_refinements:
                ...
                if (
                    feedback
                    and feedback.is_accurate
                    and feedback.is_audience_appropriate
                ):
                    break
        ...
        math_theory_data = MathTheory(
            theory_name=theory_name, explanations=explanations
        )
