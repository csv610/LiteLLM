...
def arguments_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch high-quality explanations for math theories via a 3-agent pipeline"
    )
    parser.add_argument(
        "-t", "--theory",
        type=str,
...
def fetch_theory_info(theory_name: str, model_config: ModelConfig, levels: List[AudienceLevel] = None) -> MathTheory | None:
    """Fetch information for a single mathematical theory using the 3-agent pipeline."""
...
def math_theory_to_markdown(theory_info: MathTheory) -> str:
    ...
    md.append("> *Generated via a 3-agent pipeline (Researcher, Writer, Reviewer)*\n")
...
def math_theory_cli():
    ...
    configure_logging()
    # Set logging to INFO to see agent progress
    logging.getLogger("math_theory_element").setLevel(logging.INFO)
    ...
    print(f"Starting 3-agent pipeline for '{args.theory}' at level '{args.level}'...", file=sys.stderr)
    ...
    if theory_info:
        ...
        print(f"\nSuccess! Saved explanation to {output_file}", file=sys.stderr)
    else:
